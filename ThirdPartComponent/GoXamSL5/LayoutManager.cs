...
namespace Northwoods.GoXam {
  ...
  public class LayoutManager : FrameworkElement {
    ...
    // request performing all invalid layouts
    internal void UpdateDiagramLayout() {
      // delay invoking, to reduce chance of repeated layouts during initialization/loading
      if (this.LayoutDiagramReason == null) {
        Diagram.InvokeLater(this, () => {
          InvokeLayoutDiagram("UpdateDiagramLayout");
        });
      }
    }
    ...
    public virtual void LayoutDiagram(LayoutInitial init, bool immediate) {
      if (init == LayoutInitial.ValidateAll) {
        ...
        Diagram.InvokeLater(this, () => {
          ResetAllLayouts(init);
          InvokeLayoutDiagram("LayoutDiagram"); 
        });
      }
    }
    ...
    internal void InvokeLayoutDiagram(String why) {
      VerifyAccess();
      //Diagram.Debug("InvokeLayoutDiagram " + why + " " + Diagram.Str(this.Diagram));
      if (this.LayoutDiagramReason == null) {
        ...
        //Diagram.Debug("  invoked for later");
    ...
    private bool WasSkippingUndoManager;
    private bool WasEnabled { get; set; }  // whether Diagram.IsEnabled was true when starting animation
    private bool HadFocus { get; set; }  // whether Diagram.IsKeyboardFocused was true
    ...
    private void FinishedLayout() {  // no animation
      ...
      diagram.Cursor = null;
      IDiagramModel model = diagram.Model;
      if (model != null) model.SkipsUndoManager = this.WasSkippingUndoManager;
      DiagramPanel panel = diagram.Panel;
    ...
    private HashSet<Node> ResizedNodes = new HashSet<Node>();

    private void ClearSizeChangedByLayoutFlags() {
      //Diagram.Debug("  ClearSizeChangedByLayoutFlags " + this.ResizedNodes.Count.ToString());
      foreach (Node n in this.ResizedNodes) {
        n.SizeChangedByLayout = false;
      }
    ...
    public virtual void InvalidateLayout(Part p, LayoutChange change) {
      ...
      if (layout != null) {
        bool wasvalid = layout.ValidLayout;
        layout.Invalidate(change, p);
        if (this.AutoUpdatesLayout && change != LayoutChange.GroupSizeChanged) {
          if (wasvalid && !layout.ValidLayout) {
            UpdateDiagramLayout();
          }
        }
      }
    ...
    internal bool AutoUpdatesLayout { get; set; }
    ...
    private void UpdateLinksAndFinish() {
