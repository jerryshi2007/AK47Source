...
namespace Northwoods.GoXam.Layout {
  ...
  public class DiagramLayout : FrameworkElement, IDiagramLayout {  //?? needs to be FrameworkElement to allow databinding properties, especially as Group.Layout
    ...
    public bool ValidLayout {
      ...
      set {
        ...
        if (old != value) {
          ...
          //if (!(this.Diagram is Palette) && !value) Diagram.Debug(" ValidLayout = " + value.ToString() + " " + (this.Group == null ? "(diagram)" : Diagram.Str(this.Group)));
        }
      }
    }
    ...
    public virtual void Invalidate(LayoutChange reason, Part part) {
      ...
      // no Part needed for these cases:
      if ((reason&this.Conditions&LayoutChange.DiagramLayoutChanged) == LayoutChange.DiagramLayoutChanged ||
          (reason&this.Conditions&LayoutChange.ViewportSizeChanged) == LayoutChange.ViewportSizeChanged) {
        this.ValidLayout = false;
        return;
      }
      // now require a Part
    ...
    internal Point InitialOrigin(Point origin) {
      if (this.Group != null) {
        ...
        if (gp != null) {
          loc = this.Group.GetElementPoint(gp, Spot.TopLeft);
          loc.X += gp.Padding.Left;
          loc.Y += gp.Padding.Top;
        } else {
          loc = this.Group.Position;
        }
        return loc;
      }
      return origin;
  ...
  public enum LayoutChange {
    ...
    /// <summary>
    /// The size of the <see cref="DiagramPanel.ViewportBounds"/> has changed, either because the
    /// <see cref="DiagramPanel"/> has changed size or because the <see cref="DiagramPanel.Scale"/> has changed.
    /// This only applies to the top-level layout, the <see cref="Northwoods.GoXam.Diagram.Layout"/>.
    /// </summary>
    ViewportSizeChanged=0x1000,
    /// <summary>
    ...
    /// </summary>
    All=Standard | VisibleChanged | GroupSizeChanged | NodeSizeChanged | NodeLocationChanged | ViewportSizeChanged,
    /// <summary>
