...
namespace Northwoods.GoXam {
...

 {
    ...
    internal T FindElementAt<T>(Point p, Func<

      UIElement



      , T> navig, Predicate<T> pred)
...

    {
      ...
      Point gpt = this.Panel.TransformModelToGlobal(p);
      IEnumerable<UIElement> elts = VisualTreeHelper.FindElementsInHostCoordinates(gpt, this);
      foreach (UIElement elt in elts) {
        T part = navig(elt);
        if (part != null && (pred == null || pred(part))) return part;
      }
      return null;
    ...
    internal IEnumerable<T> FindElementsAt<T>(Point p, Func<
...

    {
      ...
      foreach (UIElement elt in elts) {
        T part = navig(elt);
        if (part != null && (pred == null || pred(part))) {
          coll.Add(part);
        }
      }
  ...
  public class NodeLayer : Layer {
    ...
    internal bool IsAddingNode { get; set; }
    ...
    internal void SortZOrder(Group g) {
      if (g == null) return;
      if (g.Layer == this) MoveGroupBehind(g);
  ...
  public class LinkLayer : Layer {
    ...
    internal bool IsAddingLink { get; set; }
    ...
    public void Remove(Link part) {
      ...
      if (part.Layer == this) {
        ...
        if (part.Route.JumpsOver) {
          part.Route.InvalidateOtherJumpOvers(part.Bounds, this.Panel);
        }
        InternalRemove(part);
        part.Layer = null;  // backpointer
      }
