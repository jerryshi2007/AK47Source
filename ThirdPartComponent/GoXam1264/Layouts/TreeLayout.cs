...
namespace Northwoods.GoXam.Layout {
  ...
  public class TreeLayout : DiagramLayout {
      ...
      switch (this.Path) {
        case TreePath.Destination:
          if (v.DestinationEdgesCount > 0) {
            ...
            foreach (TreeVertex c in v.DestinationVertexes) {
              if (WalkOK(v, c)) list.Add(c);
            }
    ...
    protected virtual void AddComments(TreeVertex v) {
      ...
      TreeAlignment palign = TreeAlignment.CenterChildren;
      bool inbus = false;
      if (v.Parent != null) {
        pa = v.Parent.Angle;
        palign = v.Parent.Alignment;
        inbus = (palign == TreeAlignment.Bus || palign == TreeAlignment.TopLeftBus || palign == TreeAlignment.BottomRightBus);
      }
      bool vertical = (a == 90 || a == 270);
      bool pvertical = (pa == 90 || pa == 270);
      bool leaf = (v.ChildrenCount == 0);
      bool busparent = (align == TreeAlignment.Bus || align == TreeAlignment.TopLeftBus || align == TreeAlignment.BottomRightBus);
      bool left = IsLeftSideBus(v);
      Rect extra = new Rect();
      double spacing = 0;
      if (v.Node != null) {
        ...
        foreach (Node comment in comments) {
          ...
          comment.MaybeRemeasureNow();
          Rect cb = comment.Bounds;
          if ((vertical && !leaf) || (!inbus && !pvertical && leaf) || (inbus && pvertical && leaf)) {  // if vertical tree, leave room for comments on the side, unless it's a leaf
            extra.Width = Math.Max(extra.Width, cb.Width);
            extra.Height += cb.Height + Math.Abs(spacing);
          } else {
            extra.Width += cb.Width + Math.Abs(spacing);
      ...
      if (v.Comments != null && v.Comments.Count > 0) {
        if ((vertical && !leaf) || (!inbus && !pvertical && leaf) || (inbus && pvertical && leaf)) {
          extra.Width += Math.Abs(v.CommentMargin);
          ...
          extra.Height += Math.Abs(v.CommentMargin);
          if (pa > 135 && !inbus || (!pvertical && left)) {
            v.Focus = new Point(v.Focus.X, v.Focus.Y + extra.Height);
          }
          extra.Width = Math.Max(0, extra.Width - v.Width);
        }
        v.Bounds = new Rect(0, 0, v.Bounds.Width + extra.Width, v.Bounds.Height + extra.Height);
      }
    }

    // find if the vertex is on the left or top side of the bus
    // this method is called to find if a comment should be placed to the left or above the vertex
    private bool IsLeftSideBus(TreeVertex v) {
      ...
      if (inbus) {
        TreeVertex[] children = parent.Children;
        if (palign == TreeAlignment.Bus) {
          int i = 0;
          for (; i < children.Length; i++) {
            if (v == children[i]) break;
          }
          return (i % 2 == 0);
        } else if (palign == TreeAlignment.TopLeftBus) {
          return true;
        } else {
          return false;
        }
          ...
          if ((vertical && !leaf) || (!inbus && !pvertical && leaf) || (inbus && pvertical && leaf)) {
            if ((pa > 135 && !inbus) || (pvertical && left)) {
              if (v.CommentMargin >= 0) {
                c.Location = new Point(nodebounds.Left - v.CommentMargin - c.Bounds.Width, nodebounds.Top + f);  //??? assumes LocationSpot is TopLeft for each comment
                foreach (Link link in c.LinksInto) {
              ...
              if (v.CommentMargin >= 0) {
                ...
                c.Location = new Point(nodebounds.Left + v.CommentMargin - c.Bounds.Width, nodebounds.Top + f);
                foreach (Link link in c.LinksInto) {
                  link.Route.FromSpot = Spot.MiddleLeft;
                  link.Route.ToSpot = Spot.MiddleRight;
                }
            ...
            if (v.CommentSpacing >= 0)
              f += c.Bounds.Height + v.CommentSpacing;
            else
              f += v.CommentSpacing - c.Bounds.Height;
          } else {
            if ((pa > 135 && !inbus) || (!pvertical && left)) {
              if (v.CommentMargin >= 0) {
                c.Location = new Point(nodebounds.Left + f, nodebounds.Top - v.CommentMargin - c.Bounds.Height);
                foreach (Link link in c.LinksInto) {
            ...
            if (v.CommentSpacing >= 0)
              f += c.Bounds.Width + v.CommentSpacing;
            else
              f += v.CommentSpacing - c.Bounds.Width;
          }
    ...
    private void LayoutTreeNone(TreeVertex v) {
      ...
      if (v.ChildrenCount == 0) {
        ...
        if (v.Parent != null && v.Comments != null && (((pa == 180 || pa == 270) && !inbus) || left)) {
          if ((pa == 180 && !inbus) || ((pa == 90 || pa == 270) && left))
            v.SubtreeOffset = new Point(v.Width - v.Node.Bounds.Width, 0);
          else
            v.SubtreeOffset = new Point(0, v.Height - v.Node.Bounds.Height);
        } else {
      ...
      bool alignbus = (align == TreeAlignment.Bus || align == TreeAlignment.TopLeftBus || align == TreeAlignment.BottomRightBus);
      ...
      TreeStyle style = this.TreeStyle;
      double maxsubhalf = 0;
      if (alignbus) {
        if (angle > 135) Array.Reverse(children);
      }
      if (align == TreeAlignment.Bus) {
        for (int i = 0; i < children.Length; i++) {
          if (i % 2 != 0) continue;
          // for a bus, remember the subwidth of one half so we know where to offset the parent
          maxsubhalf = Math.Max(maxsubhalf, (vertical ? children[i].SubtreeSize.Width : children[i].SubtreeSize.Height));
        }
      }
      Point extra = new Point(v.Width - v.Node.Bounds.Width, v.Height - v.Node.Bounds.Height); // determine the extra size due to comments
      // lay out nodes in a bus
      if (alignbus) {
        switch (align) {
          case TreeAlignment.Bus:
            int i = 0;
            // set RelativePositions for the first half of the children
            for (i = 0; i < children.Length; i++) {
              if (i % 2 != 0) continue;
              TreeVertex child = children[i];
              double rspace = (col == 0 ? 0  : rowspacing);
              if (vertical) {
                // align the right edges of the vertexes so the distances between the two sides of the bus are constant
                child.RelativePosition = new Point(maxsubhalf - child.SubtreeSize.Width, y + rspace);
                subwidth = Math.Max(subwidth, child.SubtreeSize.Width);
                subheight = Math.Max(subheight, y + rspace + child.SubtreeSize.Height);
                y += rspace + child.SubtreeSize.Height;
              } else {
                // align the bottom edges of the vertexes so the distances between the two sides of the bus are constant
                child.RelativePosition = new Point(x + rspace, maxsubhalf - child.SubtreeSize.Height);
                subwidth = Math.Max(subwidth, x + rspace + child.SubtreeSize.Width);
                subheight = Math.Max(subheight, child.SubtreeSize.Height);
                x += rspace + child.SubtreeSize.Width;
              }
              col++;
            }
            ...
            col = 0;
            if (vertical) {
              x += subwidth + nodespacing;
              y = 0;
            } else {
              x = 0;
              y += subheight + nodespacing;
            }
            for (int j = 0; j < children.Length; j++) {
              if (j % 2 == 0) continue;
              TreeVertex child = children[j];
              double rspace = (col == 0 ? 0  : rowspacing);
              if (vertical) {
                child.RelativePosition = new Point(x, y + rspace);
                subwidth = Math.Max(subwidth, x + child.SubtreeSize.Width);
                subheight = Math.Max(subheight, y + rspace + child.SubtreeSize.Height);
                y += rspace + child.SubtreeSize.Height;
              } else {
                child.RelativePosition = new Point(x + rspace, y);
                subwidth = Math.Max(subwidth, x + rspace + child.SubtreeSize.Width);
                subheight = Math.Max(subheight, y + child.SubtreeSize.Height);
                x += rspace + child.SubtreeSize.Width;
              }
              col++;
            }
            break;
          case TreeAlignment.TopLeftBus:
            for (int k = 0; k < children.Length; k++) {
              TreeVertex child = children[k];
              double rspace = (col == 0 ? 0  : rowspacing);
              if (vertical) {
                // align the right edges of the vertexes so the distance to the parent's center is constant
                child.RelativePosition = new Point(maxsubbreadth - child.SubtreeSize.Width, y + rspace);
                subwidth = Math.Max(subwidth, child.SubtreeSize.Width);
                subheight = Math.Max(subheight, y + rspace + child.SubtreeSize.Height);
                y += rspace + child.SubtreeSize.Height;
              } else {
                // align the bottom edges of the vertexes so the distance to the parent's center is constant
                child.RelativePosition = new Point(x + rspace, maxsubbreadth - child.SubtreeSize.Height);
                subwidth = Math.Max(subwidth, x + rspace + child.SubtreeSize.Width);
                subheight = Math.Max(subheight, child.SubtreeSize.Height);
                x += rspace + child.SubtreeSize.Width;
              }
            ...
            break;
          case TreeAlignment.BottomRightBus:
            for (int l = 0; l < children.Length; l++) {
              TreeVertex child = children[l];
              double rspace = (col == 0 ? 0 : rowspacing);
              if (vertical) {
                // set the child's RelativePosition offset from the parent, because we don't want a negative offset later
                child.RelativePosition = new Point(nodespacing/2 + v.Node.Bounds.Width/2, y + rspace);
                subwidth = Math.Max(subwidth, child.SubtreeSize.Width);
                subheight = Math.Max(subheight, y + rspace + child.SubtreeSize.Height);
                y += rspace + child.SubtreeSize.Height;
              } else {
                child.RelativePosition = new Point(x + rspace, nodespacing/2 + v.Node.Bounds.Height/2);
                subwidth = Math.Max(subwidth, x + rspace + child.SubtreeSize.Width);
                subheight = Math.Max(subheight, child.SubtreeSize.Height);
                x += rspace + child.SubtreeSize.Width;
              }
      ...
      Node node = v.Node;
      Rect nodebounds = node.Bounds; //use the bounds of the node itself so comments aren't included
      switch (align) {
        case TreeAlignment.CenterSubtrees:
          if (vertical) {
            offset.X += (subwidth-nodebounds.Width)/2;
          } else {
            offset.Y += (subheight-nodebounds.Height)/2;
          }
          break;
        case TreeAlignment.CenterChildren:
          if (row > 0) {  // same as CenterSubtrees when there's multiple rows
            ...
            if (vertical) {
              ...
              double last = v.Children[num-1].RelativePosition.X + v.Children[num-1].SubtreeOffset.X + v.Children[num-1].Node.Bounds.Width;
              offset.X += first + (last-first-nodebounds.Width)/2;
            } else {
              double first = v.Children[0].SubtreeOffset.Y;
              double last = v.Children[num-1].RelativePosition.Y + v.Children[num-1].SubtreeOffset.Y + v.Children[num-1].Node.Bounds.Height;
              offset.Y += first + (last-first-nodebounds.Height)/2;
            }
          }
          break;
        ...
        case TreeAlignment.End:
          if (vertical) {
            subwidth += nodeindent;
            offset.X += (subwidth-nodebounds.Width);
          } else {
            subheight += nodeindent;
            offset.Y += (subheight-nodebounds.Height);
          }
          break;
        case TreeAlignment.Bus:
          if (vertical) {
            if (children.Length > 1) {
              offset.X += maxsubhalf + nodespacing/2 - nodebounds.Width/2;
            } else { // with only one child, offset just like CenterChildren
              offset.X += (v.Children[0].Node.Bounds.Width - nodebounds.Width)/2 + v.Children[0].SubtreeOffset.X;
            }
          } else {
            if (children.Length > 1) {
              offset.Y += maxsubhalf + nodespacing/2 - nodebounds.Height/2;
            } else {
              offset.Y += (v.Children[0].Node.Bounds.Height - nodebounds.Height)/2 + v.Children[0].SubtreeOffset.Y;
            }
          }
          break;
        case TreeAlignment.TopLeftBus:
          if (vertical) {
            offset.X += subwidth + nodespacing/2 - nodebounds.Width/2;
          } else {
            offset.Y += subheight + nodespacing/2 - nodebounds.Height/2;
          }
          break;
        case TreeAlignment.BottomRightBus:
          break;
        ...
        if (vertical) {
          child.RelativePosition = new Point(
            ...
            child.RelativePosition.Y + ((angle > 135) ?
              ((alignbus ? -subheight : -child.SubtreeSize.Height) + child.SubtreeOffset.Y - layerspacing) :
              (v.Height + layerspacing + child.SubtreeOffset.Y)));
        } else {
          child.RelativePosition = new Point(
            child.RelativePosition.X + ((angle > 135) ?
              ((alignbus ? -subwidth : -child.SubtreeSize.Width) + child.SubtreeOffset.X - layerspacing) :
              (v.Width + layerspacing + child.SubtreeOffset.X)),
            child.RelativePosition.Y + child.SubtreeOffset.Y - offset.Y);
        }
      }
      bool negcomment = false; // determine if the node has a comment in a negative direction
      if (v.Comments != null) negcomment = (v.Focus.X != nodebounds.Width/2 || v.Focus.Y != nodebounds.Height/2);
      // include the NODE itself in the subtree size, and adjust the offset if the direction is negative
      if (vertical) {
        if (v.Comments == null && (!alignbus || align == TreeAlignment.Bus)) subwidth = Math.Max(subwidth, v.Width);
        else subwidth = CalculateSubwidth(v, subwidth, offset.X, negcomment);
        if (negcomment && extra.X > offset.X) offset.X = extra.X;  //make sure to offset if there is a comment in the negative direction
        ...
        if (v.Comments == null && (!alignbus || align == TreeAlignment.Bus)) subheight = Math.Max(subheight, v.Height);
        else subheight = CalculateSubheight(v, subheight, offset.Y, negcomment);
        if (negcomment && extra.Y > offset.Y) offset.Y = extra.Y;
      }
      // save results
    ...
    private double CalculateSubwidth(TreeVertex v, double subwidth, double offset, bool negcomment) {
      // when comments are involved, make sure they are accounted for in subwidth if they extend beyond the edges of the subtree
      switch (v.Alignment) {
        case TreeAlignment.CenterChildren:
          if (!negcomment) {
            if (subwidth < offset + v.Width) return offset + v.Width;
            return subwidth;
          } else {
            if (subwidth < subwidth - offset - v.Node.Bounds.Width + v.Width) return subwidth - offset - v.Node.Bounds.Width + v.Width;
            return subwidth;
          }
        case TreeAlignment.CenterSubtrees:
          if (subwidth < offset + v.Width) return offset + v.Width;
        ...
        case TreeAlignment.Start:
          if (negcomment) {
            if (v.Node.Bounds.Width > subwidth) return v.Width;
            return subwidth + v.Width - v.Node.Bounds.Width;
          } else {
        ...
        case TreeAlignment.Bus:
          if (v.ChildrenCount > 1) {
            ...
            for (int i = 0; i < v.Children.Length; i++) {
              if (i % 2 != 0) continue;
              subleft = Math.Max(subleft, v.Children[i].SubtreeSize.Width);
            }
            double subright = subwidth - v.NodeSpacing - subleft;
            if (negcomment) {
              if (v.Width > v.Node.Bounds.Width/2 + v.NodeSpacing/2 + subleft)
                return v.Width - v.Node.Bounds.Width/2 + v.NodeSpacing/2 + subright;
              return subwidth;
            } else {
              if (v.Width > v.Node.Bounds.Width/2 + v.NodeSpacing/2 + subright)
                return v.Width - v.Node.Bounds.Width/2 + v.NodeSpacing/2 + subleft;
              return subwidth;
            }
          } else {
            if (!negcomment) {
        ...
        case TreeAlignment.TopLeftBus:
          if (!negcomment) return v.Width - v.Node.Bounds.Width/2 + v.NodeSpacing/2 + subwidth;
          return Math.Max(v.Width, v.Node.Bounds.Width/2 + v.NodeSpacing/2 + subwidth);
        case TreeAlignment.BottomRightBus:
          if (negcomment) return v.Width - v.Node.Bounds.Width/2 + v.NodeSpacing/2 + subwidth;
          return Math.Max(v.Width, v.Node.Bounds.Width/2 + v.NodeSpacing/2 + subwidth);
        default:
    ...
    private double CalculateSubheight(TreeVertex v, double subheight, double offset, bool negcomment) {
      // when comments are involved, make sure they are accounted for in subheight if they extend beyond the edges of the subtree
      switch (v.Alignment) {
        case TreeAlignment.CenterChildren:
          if (!negcomment) {
            if (subheight < offset + v.Height) return offset + v.Height;
            return subheight;
          } else {
            if (subheight < subheight - offset - v.Node.Bounds.Height + v.Height) return subheight - offset - v.Node.Bounds.Height + v.Height;
            return subheight;
          }
        case TreeAlignment.CenterSubtrees:
          if (subheight < offset + v.Height) return offset + v.Height;
        ...
        case TreeAlignment.Start:
          if (negcomment) {
            if (v.Node.Bounds.Height > subheight) return v.Height;
            return subheight + v.Height - v.Node.Bounds.Height;
          } else {
        ...
        case TreeAlignment.Bus:
          if (v.ChildrenCount > 1) {
            ...
            for (int i = 0; i < v.Children.Length; i++) {
              if (i % 2 != 0) continue;
              subleft = Math.Max(subleft, v.Children[i].SubtreeSize.Height);
            }
            double subright = subheight - v.NodeSpacing - subleft;
            if (negcomment) {
              if (v.Height > v.Node.Bounds.Height/2 + v.NodeSpacing/2 + subleft)
                return v.Height - v.Node.Bounds.Height/2 + v.NodeSpacing/2 + subright;
              return subheight;
            } else {
              if (v.Height > v.Node.Bounds.Height/2 + v.NodeSpacing/2 + subright)
                return v.Height - v.Node.Bounds.Height/2 + v.NodeSpacing/2 + subleft;
              return subheight;
            }
          } else {
            if (!negcomment) {
        ...
        case TreeAlignment.TopLeftBus:
          if (!negcomment) return v.Height - v.Node.Bounds.Height/2 + v.NodeSpacing/2 + subheight;
          return Math.Max(v.Height, v.Node.Bounds.Height/2 + v.NodeSpacing/2 + subheight);
        case TreeAlignment.BottomRightBus:
          if (negcomment) return v.Height - v.Node.Bounds.Height/2 + v.NodeSpacing/2 + subheight;
          return Math.Max(v.Height, v.Node.Bounds.Height/2 + v.NodeSpacing/2 + subheight);
        default:
    ...
    private void LayoutTreeBlock(TreeVertex vertex) {
      ...
      if (vertex.ChildrenCount == 0) {
        ...
        if (vertex.Parent != null) {
          pa = vertex.Parent.Angle;
          palign = vertex.Parent.Alignment;
          inbus = (palign == TreeAlignment.Bus || palign == TreeAlignment.TopLeftBus || palign == TreeAlignment.BottomRightBus);
        }
        bool left = IsLeftSideBus(vertex);
        vertex.RelativePosition = new Point(0, 0);
        vertex.SubtreeSize = vertex.Size;
        // if a comment goes in a negative direction, offset the node to account for the extra space needed for the subwidth/subheight
        if (vertex.Parent != null && vertex.Comments != null && (((pa == 180 || pa == 270) && !inbus) || left)) {
          if ((pa == 180 && !inbus) || ((pa == 90 || pa == 270) && left))
            vertex.SubtreeOffset = new Point(vertex.Width - vertex.Node.Bounds.Width, 0);
          else
            vertex.SubtreeOffset = new Point(0, vertex.Height - vertex.Node.Bounds.Height);
        } else {
      ...
      Point extra = new Point(vertex.Width - vertex.Node.Bounds.Width, vertex.Height - vertex.Node.Bounds.Height);
      // lay out nodes in a bus
      if (alignbus) {
        switch (align) {
          case TreeAlignment.Bus:
          ...
          case TreeAlignment.BottomRightBus:
            for (int l = 0; l < children.Length; l++) {
              ...
              if (vertical) {
                // set the child's RelativePosition offset from the parent, because we don't want a negative offset later
                child.RelativePosition = new Point(nodespacing/2 + vertex.Node.Bounds.Width/2, y + rspace);
                subwidth = Math.Max(subwidth, child.SubtreeSize.Width);
                subheight = Math.Max(subheight, y + rspace + child.SubtreeSize.Height);
                y += rspace + child.SubtreeSize.Height;
              } else {
                child.RelativePosition = new Point(x + rspace, nodespacing/2 + vertex.Node.Bounds.Height/2);
                subwidth = Math.Max(subwidth, x + rspace + child.SubtreeSize.Width);
                subheight = Math.Max(subheight, child.SubtreeSize.Height);
                x += rspace + child.SubtreeSize.Width;
              }
        ...
        if (vertical) {
          ...
          lfringe[1] = new Point(0, subheight);
          rfringe[0] = new Point(subwidth, 0);
          rfringe[1] = new Point(subwidth, subheight);
        } else {
          lfringe[0] = new Point(0, 0);
          lfringe[1] = new Point(subwidth, 0);
          rfringe[0] = new Point(0, subheight);
          rfringe[1] = new Point(subwidth, subheight);
        }
      } else {
        for (int idx = 0; idx < children.Length; idx++) {
      ...
      switch (align) {
        case TreeAlignment.CenterSubtrees:
          if (vertical) {
            offset.X += (subwidth-nodebounds.Width-nodeindent)/2;
          } else {
            offset.Y += (subheight-nodebounds.Height-nodeindent)/2;
          }
          break;
        case TreeAlignment.CenterChildren:
          if (row > 0) {  // same as CenterSubtrees when there's multiple rows
            ...
            if (vertical) {
              ...
              double last = vertex.Children[num-1].RelativePosition.X + vertex.Children[num-1].SubtreeOffset.X + vertex.Children[num-1].Node.Bounds.Width;
              offset.X += first + (last-first-nodebounds.Width-nodeindent)/2;
            } else {
              double first = vertex.Children[0].RelativePosition.Y + vertex.Children[0].SubtreeOffset.Y;
              double last = vertex.Children[num-1].RelativePosition.Y + vertex.Children[num-1].SubtreeOffset.Y + vertex.Children[num-1].Node.Bounds.Height;
              offset.Y += first + (last-first-nodebounds.Height-nodeindent)/2;
            }
          }
          break;
        case TreeAlignment.Start:
        ...
        case TreeAlignment.End:
          if (vertical) {
            offset.X += (subwidth-nodebounds.Width)+nodeindent;
            subwidth += nodeindent;
          } else {
            offset.Y += (subheight-nodebounds.Height)+nodeindent;
            subheight += nodeindent;
          }
          break;
        case TreeAlignment.Bus:
          if (vertical) {
            if (children.Length > 1) offset.X += maxsubhalf + nodespacing/2 - nodebounds.Width/2;
            // with only one child, offset just like CenterChildren
            else offset.X += (vertex.Children[0].Node.Bounds.Width - nodebounds.Width)/2 + vertex.Children[0].SubtreeOffset.X;
          } else {
            if (children.Length > 1) offset.Y += maxsubhalf + nodespacing/2 - nodebounds.Height/2;
            else offset.Y += (vertex.Children[0].Node.Bounds.Height - nodebounds.Height)/2 + vertex.Children[0].SubtreeOffset.Y;
          }
          break;
        case TreeAlignment.TopLeftBus:
          if (vertical) offset.X += subwidth + nodespacing/2 - nodebounds.Width/2;
          else offset.Y += subheight + nodespacing/2 - nodebounds.Height/2;
          break;
        case TreeAlignment.BottomRightBus:
          break;
        default:
      ...
      bool negcomment = false; // determine if the node has a comment in a negative direction
      if (vertex.Comments != null) negcomment = (vertex.Focus.X != nodebounds.Width/2 || vertex.Focus.Y != nodebounds.Height/2);
      // include the NODE itself in the subtree size, and adjust the offset if the direction is negative
      Size fringeoff = new Size(0, 0);
      double tempsubwidth = subwidth;
      if (alignbus) {
        if (vertical) {
          if (align == TreeAlignment.Bus && vertex.Comments == null) subwidth = Math.Max(subwidth, vertex.Width);
          else subwidth = CalculateSubwidth(vertex, subwidth, offset.X, negcomment);
          if (negcomment && extra.X > offset.X) offset.X = extra.X;  //make sure to offset if there is a comment in the negative direction
          if (angle > 135) offset.Y += subheight + layerspacing;
          subheight = Math.Max(Math.Max(subheight, vertex.Height), subheight + vertex.Height + layerspacing);
          if (align == TreeAlignment.BottomRightBus)
            fringeoff.Width += nodespacing/2 + nodebounds.Width/2;
          fringeoff.Height += vertex.Height + layerspacing;
        } else {
          if (align == TreeAlignment.Bus && vertex.Comments == null) subheight = Math.Max(subheight, vertex.Height);
          else subheight = CalculateSubheight(vertex, subheight, offset.Y, negcomment);
          if (negcomment && extra.Y > offset.Y) offset.Y = extra.Y;
          if (angle > 135) offset.X += subwidth + layerspacing;
          subwidth = Math.Max(Math.Max(subwidth, vertex.Width), subwidth + vertex.Width + layerspacing);
          if (align == TreeAlignment.BottomRightBus)
            fringeoff.Height += nodespacing/2 + nodebounds.Height/2;
          fringeoff.Width += vertex.Width + layerspacing;
        }
      } else {
        if (vertical) {
          if (vertex.Comments == null) {
            ...
            subwidth = CalculateSubwidth(vertex, subwidth, offset.X, negcomment);
          }
          if (offset.X < 0) {
          ...
          if (vertex.Comments == null) {
            ...
            subheight = CalculateSubheight(vertex, subheight, offset.Y, negcomment);
          }
          if (offset.Y < 0) {
      ...
      if (row > 0) {  // when there are multiple rows, just assume everything's a big block
        ...
        if (vertical) {
          newleft[2]  = new Point(0, (negcomment ? vertex.Node.Bounds.Height : vertex.Height) + layerspacing);
          ...
          newleft[2]  = new Point((negcomment ? vertex.Node.Bounds.Width : vertex.Width) + layerspacing, 0);
      ...
      if (vertical) {
        newleft[0]  = new Point((negcomment ? 0 : offset.X), 0);  // include negative comments in fringes
        ...
        newright[0] = new Point((negcomment ? 0 : offset.X) + vertex.Width, 0);
        ...
        newleft[0]  = new Point(0, (negcomment ? 0 : offset.Y));
        ...
        newright[0] = new Point(0, (negcomment ? 0 : offset.Y) + vertex.Height);
    ...
    protected virtual void SetPortSpots(TreeVertex v) {
      TreeAlignment align = v.Alignment;
      if (align == TreeAlignment.Bus || align == TreeAlignment.TopLeftBus || align == TreeAlignment.BottomRightBus) {
        SetPortSpotsBus(v, align);
      } else {
        double angle = OrthoAngle(v);
    ...
    // set the PortSpots for vertexes with Bus alignments
    private void SetPortSpotsBus(TreeVertex v, TreeAlignment align) {
      bool dest = (this.Path == TreePath.Destination);
      double angle = OrthoAngle(v);
      Spot parentspot;
      if (angle == 0) parentspot = Spot.MiddleRight;
      else if (angle == 90) parentspot = Spot.MiddleBottom;
      else if (angle == 180) parentspot = Spot.MiddleLeft;
      else parentspot = Spot.MiddleTop;
      switch (align) {
        case TreeAlignment.Bus: {
            int i = 0;
            foreach (TreeVertex c in v.Children) {
              TreeEdge edge = (dest ? c.SourceEdges.FirstOrDefault() : c.DestinationEdges.FirstOrDefault());
              if (edge == null) continue;
              Link l = edge.Link;
              if (l != null) {
                Spot childspot = (angle == 90 || angle == 270 ? Spot.MiddleLeft : Spot.MiddleTop);
                if (v.ChildrenCount == 1) {
                  if (angle == 0) childspot = Spot.MiddleLeft;
                  else if (angle == 90) childspot = Spot.MiddleTop;
                  else if (angle == 180) childspot = Spot.MiddleRight;
                  else childspot = Spot.MiddleBottom;
                } else {
                  if (i % 2 == 0) {
                ...
                if (dest) {
                  l.Route.FromSpot = parentspot;
                  l.Route.ToSpot = childspot;
                } else {
                  l.Route.FromSpot = childspot;
                  l.Route.ToSpot = parentspot;
                }
                i++;
              }
            }
        ...
        case TreeAlignment.TopLeftBus: {
            Spot childspot = (angle == 90 || angle == 270 ? Spot.MiddleRight : Spot.MiddleBottom);
            foreach (TreeEdge edge in (dest ? v.DestinationEdges : v.SourceEdges)) {
              Link l = edge.Link;
              if (l != null) {
                if (dest) {
                  l.Route.FromSpot = parentspot;
  ...
  public enum TreeAlignment {
    ...
    End,
    /// <summary>
    /// The children are positioned in a bus with the parent centered above or below.
