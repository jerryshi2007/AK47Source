...
namespace Northwoods.GoXam.Layout {
  ...
  public class TreeEdge : TreeNetwork.Edge {
    ...
    private void AdjustRouteForAngleChange(TreeVertex parent, TreeVertex child) {
      ...
      bool bezier = s.Curve == LinkCurve.Bezier;
      bool ortho = s.Orthogonal;
      bool alignbus = this.FromVertex.Alignment == TreeAlignment.Bus || this.FromVertex.Alignment == TreeAlignment.TopLeftBus ||
        this.FromVertex.Alignment == TreeAlignment.BottomRightBus;
      if (angle == 0) {
        ...
        if (bezier) {  // curved segments
          ...
          if (alignbus) {
            s.SetPoint(3, new Point(s.GetPoint(2).X, s.GetPoint(4).Y));
          } else if (s.PointsCount == 6) {
