...
namespace Northwoods.GoXam.Tool {
  ...
  public class ResizingTool : DiagramTool {
    ...
    public override void DoActivate() {
      ...
      Node anode = this.AdornedNode;
      FrameworkElement selelt = this.AdornedNode.SelectionElement;  // use node's angle, not AdornedElement's rotation
      this.Angle = anode.GetAngle(selelt);
      Size effSize = anode.GetEffectiveSize(this.AdornedElement);
      Point center = anode.GetElementPoint(this.AdornedElement, Spot.Center);
      this.OriginalBounds = new Rect(center.X-effSize.Width/2, center.Y-effSize.Height/2, effSize.Width, effSize.Height);
      this.OriginalLocation = anode.Location;
    ...
    protected virtual Rect ComputeResize(Point newPoint, Spot spot, Size min, Size max, bool reshape) {
      if (spot.IsNoSpot) spot = Spot.Center;
      // do all operations on unrotated rectangle
      Point oldPoint = Geo.RotatePoint(newPoint, Spot.Center.PointInRect(this.OriginalBounds), -this.Angle);
      ...
      newPoint = DraggingTool.FindNearestInfiniteGridPoint(oldPoint, new Point(0, 0), new Point(left, top), cellSize);
