...
namespace Northwoods.GoXam.Tool {
  ...
  public class DraggingTool : DiagramTool {
    ...
    static DraggingTool() {
      ...
      DropOntoEnabledProperty = DependencyProperty.Register("DropOntoEnabled", typeof(bool), typeof(DraggingTool),
        new FrameworkPropertyMetadata(false));
      DraggableLinksProperty = DependencyProperty.Register("DraggableLinks", typeof(bool), typeof(DraggingTool),
        new FrameworkPropertyMetadata(false));
    ...
    /// <summary>
    /// Identifies the <see cref="DraggableLinks"/> dependency property.
    /// </summary>
    public static readonly DependencyProperty DraggableLinksProperty;
    /// <summary>
    /// Gets or sets whether the user can drag a single <see cref="Link"/>,
    ...
    /// </value>
    /// <remarks>
    /// This affects the behavior of <see cref="DropOnto"/> and <see cref="DragOver"/>.
    /// </remarks>
    public bool DraggableLinks {
      get { return (bool)GetValue(DraggableLinksProperty); }
      set { SetValue(DraggableLinksProperty, value); }
    }
    ...
    /// <summary>
    /// Gets or sets the <see cref="Link"/> that may be dragged and reconnected
    /// when <see cref="DraggableLinks"/> is true.
    /// </summary>
    /// <value>
    /// This property is set during <see cref="DoActivate"/>.
    /// </value>
    protected Link DraggedLink { get; set; }
    ...
    public override bool CanStart() {
      ...
      //Palette: if (diagram.IsReadOnly && diagram.AllowDragOut) return true;
    ...
    private void CheckForDraggedLink(IEnumerable<Part> parts) {
      if (this.DraggableLinks) {
        Diagram diagram = this.Diagram;
        if (diagram == null || !diagram.AllowRelink) return;
        if (diagram.Model is ILinksModel && parts.Count() == 1) {
          this.DraggedLink = parts.First() as Link;
          if (this.DraggedLink != null &&
              this.DraggedLink.CanRelinkFrom() && this.DraggedLink.CanRelinkTo()) {
            this.DraggedLink.WithoutAdornments = true;
    ...
    //Palette: private bool WasReadOnly { get; set; }
    //Palette: private bool WasModifiable { get; set; }

    ///// <summary>
    ///// Clean-up side-effects of <see cref="DoActivate"/>.
    ///// </summary>
    //Palette: public override void DoDeactivate() {
    //  Diagram diagram = this.Diagram;
    //  if (this.WasReadOnly) {
    //    this.WasReadOnly = false;
    //    diagram.IsReadOnly = true;
    //    if (diagram.Model != null) diagram.Model.Modifiable = this.WasModifiable;
    //  }
    //  base.DoDeactivate();
    //}
    ...
    public virtual Dictionary<Part, Info> ComputeEffectiveCollection(IEnumerable<Part> parts) {
      ...
      if (this.DraggedLink != null && this.DraggableLinks) return map;
    ...
    private void SuspendRouting(Dictionary<Part, Info> parts) {
      ...
      //Diagram.Debug("SuspendRouting " + this.Diagram.Name);
    ...
    private void ResumeRouting(Dictionary<Part, Info> parts) {
      ...
      //Diagram.Debug("ResumeRouting " + this.Diagram.Name);
      foreach (Link link in parts.Keys.OfType<Link>()) {
        Route route = link.Route;
        if (route != null) {
    ...
    private void CleanUpDraggedLink() {
      if (this.DraggedLink != null) {
        if (this.DraggableLinks) {
          RelinkingTool tool = this.Diagram.RelinkingTool;
          if (tool != null) tool.StopDraggingMouseMove();
        }
        this.DraggedLink.WithoutAdornments = false;
        this.DraggedLink.UpdateAdornments();
        this.DraggedLink = null;
      }
    }
    ...
    /// one of the parts in the <see cref="DraggedParts"/> or <see cref="CopiedParts"/> collections.
    ...
    protected virtual void DragOver(Point pt, bool moving, bool copying) {
      ...
      if (this.DraggableLinks) {
        ...
        if (this.DraggedLink != null) {
          ILinksModel lmodel = this.DraggedLink.Model as ILinksModel;
          if (lmodel != null) {
            if (this.DraggedLink.FromNode != null) lmodel.SetLinkFromPort(this.DraggedLink.Data, null, null);
            if (this.DraggedLink.ToNode != null) lmodel.SetLinkToPort(this.DraggedLink.Data, null, null);
          }
        }
        // then see if either or both ends are over any valid ports
        UpdateDraggedLinkOverPorts(false);
      }
    ...
    private FrameworkElement FindClosestValidPort(Point p, bool toend) {
      Diagram diagram = this.Diagram;
      RelinkingTool tool = diagram.RelinkingTool;
      double gravity = tool.PortGravity;
      IEnumerable<FrameworkElement> nearports;
      if (toend) {
        nearports = diagram.Panel.FindElementsNear<FrameworkElement>(p, gravity, tool.FindValidToPort, x => true, SearchLayers.Nodes);
      } else {
        nearports = diagram.Panel.FindElementsNear<FrameworkElement>(p, gravity, tool.FindValidFromPort, x => true, SearchLayers.Nodes);
      }
    ...
    private void UpdateDraggedLinkOverPorts(bool commit) {
      Link link = this.DraggedLink;
      if (link != null) {
        Route route = link.Route;
        Diagram diagram = this.Diagram;
        DiagramPanel panel = diagram.Panel;
        RelinkingTool tool = diagram.RelinkingTool;
        // see if either (or both) ends of the link are over a valid port
        FrameworkElement maybefromport = null;
        if (tool != null && link.FromNode == null) {
          maybefromport = FindClosestValidPort(route.GetPoint(0), false);
        }
        FrameworkElement maybetoport = null;
        if (tool != null && link.ToNode == null) {
          maybetoport = FindClosestValidPort(route.GetPoint(route.PointsCount-1), true);
        }
        Node maybefromnode = Part.FindAncestor<Node>(maybefromport);
        Node maybetonode = Part.FindAncestor<Node>(maybetoport);
        bool ok = (tool != null ? tool.IsValidLink(maybefromnode, maybefromport, maybetonode, maybetoport) : true);
        //Diagram.Debug((ok ? "OK" : "invalid") +
        //              " from " + Diagram.Str(maybefromnode) + " in " + Diagram.Str(maybefromport) +
        //              "  to " + Diagram.Str(maybetonode) + " in " + Diagram.Str(maybetoport));
        if (commit) {
          ILinksModel lmodel = diagram.Model as ILinksModel;
          if (lmodel != null) {
            ...
            if (maybefromnode != null) fromdata = maybefromnode.Data;
            if (maybefromport != null) fromid = Node.GetPortId(maybefromport);
            if (maybetonode != null) todata = maybetonode.Data;
            if (maybetoport != null) toid = Node.GetPortId(maybetoport);
            // remember hints for where the ends should be
            /* if (maybefromnode == null) */ route.DefaultFromPoint = route.GetPoint(0);
            /* if (maybetonode == null) */ route.DefaultToPoint = route.GetPoint(route.PointsCount-1);
            link.Route.SuspendsRouting = false;
            lmodel.SetLinkFromPort(link.Data, fromdata, fromid);
            lmodel.SetLinkToPort(link.Data, todata, toid);
          }
        } else {
          if (ok) {
            tool.DoDraggingMouseMove(maybefromnode, maybefromport, maybetonode, maybetoport);  // some might be null
          } else {
            tool.DoDraggingMouseMove(null, null, null, null);
          }
        }
      }
    }
    ...
    protected virtual void DropOnto(Point pt) {
      ...
      if (this.DraggableLinks) UpdateDraggedLinkOverPorts(true);
    ...
    protected virtual bool MayCopy() {
      ...
      if (this.DraggedLink != null && this.DraggableLinks && this.DraggedLink.CanCopy()) return true;
      return false;
    }
    ...
    protected virtual bool MayMove() {
      ...
      if (this.DraggedLink != null && this.DraggableLinks && this.DraggedLink.CanMove()) return true;
      return false;
    }
    ...
    internal /*?? protected virtual */ void DoDragOut() {
      ...
      UIElement ve = null;
      // handle cases where the Diagram is in a visual tree where there's no Parent reference to the Application's RootVisual
      if (Application.Current != null) ve = Application.Current.RootVisual;
      if (ve == null) {
        ve = this.Diagram;
        while (ve != null) {
  ...
  public enum EffectiveCollectionInclusions {
    ...
    /// unless <see cref="Northwoods.GoXam.Diagram.TreePath"/> is <see cref="Northwoods.GoXam.Layout.TreePath.Source"/>,
