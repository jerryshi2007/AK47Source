...
namespace Northwoods.GoXam {
  ...
  public struct Spot : IFormattable {
    ...
    internal static Spot ConvertFromString(string source, IFormatProvider provider) {
      ...
      if (i < subs.Length) x = Convert.ToDouble(subs[i++], provider);
      while (i < subs.Length && subs[i].Length == 0) i++;
      if (i < subs.Length) y = Convert.ToDouble(subs[i++], provider);
      while (i < subs.Length && subs[i].Length == 0) i++;
      if (i < subs.Length) offx = Convert.ToDouble(subs[i++], provider);
      while (i < subs.Length && subs[i].Length == 0) i++;
      if (i < subs.Length) offy = Convert.ToDouble(subs[i++], provider);
