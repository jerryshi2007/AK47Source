...
namespace Northwoods.GoXam.Tool {
  ...
  public abstract class DiagramTool : FrameworkElement, IDiagramTool {
    ...
    static DiagramTool() {
      MouseEnabledProperty = DependencyProperty.Register("MouseEnabled", typeof(bool), typeof(DiagramTool), new FrameworkPropertyMetadata(true));
    }
    ...
    protected virtual Part FindPartAt(Point p, bool selectable) {
      ...
      if (selectable)
        ...
        return diagram.Panel.FindElementAt<Part>(p, Diagram.FindAncestor<Part>, x => true, SearchLayers.All);
    }
