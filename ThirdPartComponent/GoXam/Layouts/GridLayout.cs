  ...
  public class GridLayout : DiagramLayout {
    ...
    private void DoPositionLayout(Node[] nodearray, double xlimit, int climit, double spacex, double spacey) {
      ...
      foreach (Node n in nodearray) {
        ...
        switch (this.Arrangement) {
          case GridArrangement.RightToLeft: needed = Math.Abs(x-sz.Width); break;
          default:                          needed = x+sz.Width; break;
        }
        ...
        n.Move(new Point(x+coff, y), true);
    ...
    private void DoLocationLayout(Node[] nodearray, double xlimit, int climit, double spacex, double spacey) {
      ...
      foreach (Node n in nodearray) {
        ...
        if (rowcount > 0) {
          switch (this.Arrangement) {
            case GridArrangement.RightToLeft: x = Math.Floor((x-(sz.Width-relloc.X))/cellW)*cellW; break;
            default: x = Math.Ceiling((x+relloc.X)/cellW)*cellW; break;
          }
        } else {
          //indent = Math.Max(indent, relloc.X);
        }
        // maybe need to wrap to next row
        double needed;
        // subtract the stuff sticking out beyond the end of the grid from the XLIMIT
        switch (this.Arrangement) {
          case GridArrangement.RightToLeft: needed = Math.Abs(x-sz.Width+relloc.X) + cellR; break;
          default: needed = x+sz.Width-relloc.X + cellL;  break;  //???
        }
        // check the column limit or the width limit or both
        //Diagram.Debug(xlimit.ToString() + " " + needed.ToString() + " x: " + x.ToString() + " " + sz.Width.ToString() + " " + relloc.X.ToString() + " " + n.ToString());
        if ((climit > 0 && rowcount > climit-1) || (xlimit > 0 && rowcount > 0 && needed > xlimit)) {
          rowcount = 0;
          x = originx;
          y += rowheight + cellT;
        ...
        n.Move(new Point(x-relloc.X, y-relloc.Y), true);
        // advance to next column
        switch (this.Arrangement) {
          case GridArrangement.RightToLeft: x -= relloc.X; break;
          default:                          x += sz.Width-relloc.X; break;
        }
        rowcount++;
      }
    }
