...
namespace Northwoods.GoXam.Model {
  ...
  public class UndoManager {
    ...
    public UndoManager() {
      ...
      this.ChecksTransactionLevel = true;
    }
    ...
    /// </summary>
    /// <value>This will be a <c>List</c> of <see cref="UndoManager.CompoundEdit"/>s</value>
    //[TypeConverter(typeof(ExpandableObjectConverter))]
    ...
    protected virtual bool EndTransaction(bool commit, String tname) {
      ...
      //ModelHelper.Trace(this.TransactionLevel, (commit ? "commit: " : "rollback: ") + (tname != null ? tname : "(unknown)") + " " +
      //  (current != null ? current.Edits.Count.ToString() : "(no edit)") + " " + this.CompoundEdits.Count.ToString());
      if (toplevel) {
        if (commit) {
    ...
    public sealed class CompoundEdit : IUndoableEdit {  // nested class
      ...
      public override string ToString() {
        String s = "CompoundEdit: " + (this.Name != null ? this.Name : "(noname)") + " " +
          ...
          (this.IsComplete ? "complete" : "editing");
