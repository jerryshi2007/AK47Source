	...
	public class MaterialAdapter
	{
		...
		internal MaterialList LoadCopyVersionMaterial(MaterialList materials)
		{
			...
			if (materials.Count != 0)
			{
				...
				foreach (Material material in materials)
				{
					...
					sb.Append(string.Format("({0})", whereSqlClause.ToSqlString(TSqlBuilder.Instance)));
				...
				sb.Append(" AND " + whereSqlClause);

				string sql
					= string.Format("SELECT * FROM WF.MATERIAL WHERE {0}", sb.ToString());
		...
		public MaterialList LoadMaterialsByResourceID(params string[] resourceIDs)
		{
			...
			if (resourceIDs.Length != 0)
			{
				...
				string sql = string.Format("SELECT * FROM WF.MATERIAL WHERE RESOURCE_ID {0} AND VERSION_TYPE = '{1}' ORDER BY {2}",
		...
		public MaterialList LoadMaterialByMaterialID(string materialID)
		{
			...
			WhereSqlClauseBuilder sb = new WhereSqlClauseBuilder();

			sb.AppendItem<string>("ID", materialID);

			string sql = "SELECT * FROM WF.MATERIAL WHERE " + sb.ToSqlString(TSqlBuilder.Instance);
		...
		{
			var deltaMaterialsList = commonContainer.ToList();
		...
		private string GetCopyFilesSql(DeltaMaterialList deltaMaterials)
		{
			...
			foreach (Material material in deltaMaterials.Updated)
			{
				...
				strB.Append(string.Format(
						@"IF EXISTS(SELECT * FROM WF.MATERIAL WHERE PARENT_ID = {0} AND WF_ACTIVITY_ID = {1} AND VERSION_TYPE ={2})
							{3}
						  ELSE
							{4}",
							 TSqlBuilder.Instance.CheckQuotationMark(material.ParentID, true),
							 TSqlBuilder.Instance.CheckQuotationMark(material.WfActivityID, true),
							 Convert.ToInt16(MaterialVersionType.CopyVersion).ToString(),
