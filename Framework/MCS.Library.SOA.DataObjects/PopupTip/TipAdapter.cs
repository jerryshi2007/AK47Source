...
{   
	public sealed class TipAdapter : UpdatableAndLoadableAdapterBase<Tip,TipCollection>
    {
        ...
        {
            var collection = Load(o => o.AppendItem("TIP_ID", tipID));
	    ...
	    public TipCollection GetTips(string[] codeArray,string cultureName)
		{
            ...
            var builder = new InSqlClauseBuilder();
            ...
            if (builder.Count > 0)
            {
                var sql = string.Format("SELECT * FROM KB.TIP WHERE ENABLE ='1' AND CULTURE='{1}' AND CODE_NAME {0} ", builder.ToSqlStringWithInOperator(TSqlBuilder.Instance), cultureName);
