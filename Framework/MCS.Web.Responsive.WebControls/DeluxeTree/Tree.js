$HGRootNS.DeluxeTree = function (element) {
	...
	this._rootNode._element.className = "tree";
	...
	//childNodeContainer.className = "tree-folder";
...
$HGRootNS.DeluxeTree.prototype =
{
	...
	loadClientState: function (value) {
		if (value) {
			if (value != "") {
				...
				var nodes = data[2];
				this._nodes = nodes;
				Sys.UI.DomEvent.addHandler(this.get_element(), "selectstart", new Function("return false;"));
	...
	get_cloneableProperties: function () {
		var baseProperties = $HGRootNS.DeluxeTree.callBaseMethod(this, "get_cloneableProperties");
		var currentProperties = ["nodeOpenImg", "nodeCloseImg", "defaultWaitingForImage", "expandImage", "defaultExpandImage",
				"collapseImage", "defaultCollapseImage", "nodeIndent", "supportPostBack", "callBackContext"];
	...
	_prepareCloneablePropertyValues: function (newElement) {
		var properties = $HGRootNS.DeluxeTree.callBaseMethod(this, "_prepareCloneablePropertyValues");

		var doc = $HGDomElement.get_currentDocument();

		var newClientState = doc.createElement("input");
		newClientState.type = "hidden";
		newClientState.id = newClientState.uniqueID;
		newClientState.value = this.get_clientStateField().value;
		newElement.appendChild(newClientState);

		properties["clientStateField"] = newClientState;

		return properties;
	},

	cloneComponent: function (element) {
		var newTree = $HGRootNS.DeluxeTree.callBaseMethod(this, "cloneComponent", [element]);

		var nodeRoot = this._rootNode._toCanSerializedObject(true);

		newTree._dataBind(nodeRoot.nodes);
	...
	raiseNodeCheckBoxBeforeClick: function (node, eventElement) {
		...
		if (handlers) {
			...
			e.cancel = false;
			e.eventElement = eventElement;
			e.eventElement.checked = !node.get_checked();
	...
	add_beforeAllDataBind: function (handler) {
		this.get_events().addHandler("beforeAllDataBind", handler);
	},

	remove_beforeAllDataBind: function (handler) {
		this.get_events().removeHandler("beforeAllDataBind", handler);
	},

	raiseBeforeAllDataBind: function () {
		var handlers = this.get_events().getHandler("beforeAllDataBind");
		var continueExec = true;
	...
	add_afterAllDataBind: function (handler) {
		this.get_events().addHandler("afterAllDataBind", handler);
	},

	remove_afterAllDataBind: function (handler) {
		this.get_events().removeHandler("afterAllDataBind", handler);
	},

	raiseAfterAllDataBind: function () {
		var handlers = this.get_events().getHandler("afterAllDataBind");
