...
Type.registerNamespace($HGRootNSName + ".Animation");

// Create an alias for the namespace to save 25 chars each time it's used since
// this is a very long script and will take awhile to download
var $HGA = $HGRootNS.Animation;

$HGA.registerAnimation = function(name, type) {
    /// <summary>
    ...
    /// The type of the new animation must inherit from <see cref=$HBRootNSName + ".Animation.Animation" />.
    /// </param>
    /// <returns />

    // Make sure the type inherits from AjaxControlToolkit.Animation.Animation
    if (type && ((type === $HGA.Animation) || (type.inheritsFrom && type.inheritsFrom($HGA.Animation)))) {
        ...
        $HGA.__animations[name.toLowerCase()] = type;
        ...
        throw Error.argumentType('type', type, $HGA.Animation, AjaxControlToolkit.Resources.Animation_InvalidBaseType);
    }
}

$HGA.buildAnimation = function(json, defaultTarget) {
    /// <summary>
    ...
    /// </param>
    /// <returns type=$HBRootNSName + ".Animation.Animation" mayBeNull="true">
    ...
    return $HGA.createAnimation(obj, defaultTarget);    
}

$HGA.createAnimation = function(obj, defaultTarget) {
    /// <summary>
    /// The <code>createAnimation</code> function builds a new
    /// <see cref=$HBRootNSName + ".Animation.Animation" /> instance from an object
    /// that describes it.
    /// </summary>
    ...
    /// <see cref=$HBRootNSName + ".Animation.ParallelAnimation" /> and
    /// <see cref=$HBRootNSName + ".Animation.SequenceAnimation" />). The elements of
    /// the <code>AnimationChildren</code> array are valid
    /// <see cref=$HBRootNSName + ".Animation.Animation" /> objects that meet these same
    /// requirements.  In order for an animation to support child animations, it must
    /// derive from the <see cref=$HBRootNSName + ".Animation.ParentAnimation" /> class
    ...
    /// </param>
    /// <returns type=$HBRootNSName + ".Animation.Animation">
    /// <see cref=$HBRootNSName + ".Animation.Animation" /> created from the description
    /// </returns>
    /// <remarks>
    ...
    var type = $HGA.__animations[obj.AnimationName.toLowerCase()];
    if (!type) {
    ...
    if (obj.AnimationChildren && obj.AnimationChildren.length) {
        if ($HGA.ParentAnimation.isInstanceOfType(animation)) {
            for (var i = 0; i < obj.AnimationChildren.length; i++) {
                var child = $HGA.createAnimation(obj.AnimationChildren[i]);
                if (child) {
                    animation.add(child);
                }
            }
...
$HGA.Animation = function(target, duration, fps) {
    /// <summary>
    ...
    $HGA.Animation.initializeBase(this);
...
$HGA.Animation.prototype = {
    dispose : function() {
        ...
        $HGA.Animation.callBaseMethod(this, 'dispose');
    },
    ...
    setOwner : function(owner) {
        ...
        /// </summary>
        /// <param name="owner" type=$HBRootNSName + ".Animation.Animation">
        /// Parent animation
        /// </param>
        /// <returns />
        this._owner = owner;
...
$HGA.Animation.registerClass($HGRootNSName + '.Animation.Animation', Sys.Component);
$HGA.registerAnimation('animation', $HGA.Animation);


$HGA.ParentAnimation = function(target, duration, fps, animations) {
    /// <summary>
    /// The <code>ParentAnimation</code> serves as a base class for all animations that contain children (such as
    /// <see cref=$HBRootNSName + ".Animation.ParallelAnimation" />, <see cref=$HBRootNSName + ".SequenceAnimation" />,
    /// etc.).  It does not actually play the animations, so any classes that inherit from it must do so.  Any animation
    /// that requires nested child animations must inherit from this class, although it will likely want to inherit off of
    /// <see cref=$HBRootNSName + ".Animation.ParallelAnimation" /> or <see cref=$HBRootNSName + ".SequenceAnimation" />
    ...
    /// Number of steps per second.  The default is 25.
    /// </param>
    /// <param name="animations" mayBeNull="true" optional="true" parameterArray="true" elementType=$HBRootNSName + ".Animation.Animation">
    /// Array of child animations to be played
    /// </param>
    /// <animation>Parent</animation>
    $HGA.ParentAnimation.initializeBase(this, [target, duration, fps]);
...
$HGA.ParentAnimation.prototype = {
    initialize : function() {
    	/// <summary>
        /// Initialize the parent along with any child animations that have not yet been initialized themselves
    	/// </summary>
    	/// <returns />
        $HGA.ParentAnimation.callBaseMethod(this, 'initialize');
    ...
    dispose : function() {
        ...
        $HGA.ParentAnimation.callBaseMethod(this, 'dispose');
    },
    
    get_animations : function() {
    	/// <value elementType=$HBRootNSName + ".Animation.Animation">
        /// Array of child animations to be played (there are no assumptions placed on order because it will matter for some
        /// derived animations like <see cref=$HBRootNSName + ".Animation.SequenceAnimation" />, but not for
        /// others like <see cref=$HBRootNSName + ".Animation.ParallelAnimation" />).  To manipulate the child
    ...
    add : function(animation) {
    	...
    	/// </summary>
    	/// <param name="animation" type=$HBRootNSName + ".Animation.Animation">Child animation to add</param>
    	/// <returns />
    ...
    remove : function(animation) {
        ...
        /// </summary>
        /// <param name="animation" type=$HBRootNSName + ".Animation.Animation">
...
$HGA.ParentAnimation.registerClass($HGRootNSName + '.Animation.ParentAnimation', $HGA.Animation);
$HGA.registerAnimation('parent', $HGA.ParentAnimation);


$HGA.ParallelAnimation = function(target, duration, fps, animations) {
    /// <summary>
    /// The <code>ParallelAnimation</code> plays several animations simultaneously.  It inherits from
    /// <see cref=$HBRootNSName + ".Animation.ParentAnimation" />, but makes itself the owner of all
    ...
    $HGA.ParallelAnimation.initializeBase(this, [target, duration, fps, animations]);
}
$HGA.ParallelAnimation.prototype = {
    add : function(animation) {
    	/// <summary>
    	...
    	/// <param name="animation" type=$HBRootNSName + ".Animation.Animation">Child animation to add</param>
    	/// <returns />
        $HGA.ParallelAnimation.callBaseMethod(this, 'add', [animation]);
    ...
    onStart : function() {
        ...
        $HGA.ParallelAnimation.callBaseMethod(this, 'onStart');
    ...
    onEnd : function() {
        ...
        $HGA.ParallelAnimation.callBaseMethod(this, 'onEnd');
    }
}
$HGA.ParallelAnimation.registerClass($HGRootNSName + '.Animation.ParallelAnimation', $HGA.ParentAnimation);
$HGA.registerAnimation('parallel', $HGA.ParallelAnimation);


$HGA.SequenceAnimation = function(target, duration, fps, animations, iterations) {
    /// <summary>
    ...
    /// <code>SequenceAnimation</code> cannot be a child of a <see cref=$HBRootNSName + ".Animation.ParallelAnimation" />
    ...
    $HGA.SequenceAnimation.initializeBase(this, [target, duration, fps, animations]);
...
$HGA.SequenceAnimation.prototype = {
    dispose : function() {
    	/// <summary>
        /// Dispose the animation
        /// </summary>
        /// <returns />
        this._handler = null;
        $HGA.SequenceAnimation.callBaseMethod(this, 'dispose');
    },
    ...
    onStart : function() {
        ...
        /// </summary>
        /// <returns />
        $HGA.SequenceAnimation.callBaseMethod(this, 'onStart');
    ...
    onStep : function(percentage) {
        ...
        /// has been nested inside an <see cref=$HBRootNSName + ".Animation.ParallelAnimation" /> (or a derived type).
        /// </summary>
        /// <param name="percentage" type="Number">Percentage of the animation already complete</param>
        /// <returns />
    ...
    onEnd : function() {
        ...
        /// <returns />
        this._remainingIterations = 0;
        $HGA.SequenceAnimation.callBaseMethod(this, 'onEnd');
    },
...
$HGA.SequenceAnimation.registerClass($HGRootNSName + '.Animation.SequenceAnimation', $HGA.ParentAnimation);
$HGA.registerAnimation('sequence', $HGA.SequenceAnimation);


$HGA.SelectionAnimation = function(target, duration, fps, animations) {
    /// <summary>
    ...
    /// functional implementation, so consider using <see cref=$HBRootNSName + ".Animation.ConditionAnimation" /> or
    /// <see cref=$HBRootNSName + ".Animation.CaseAnimation" /> instead.
    /// </summary>
    /// <param name="target" type="Sys.UI.DomElement" mayBeNull="true" optional="true" domElement="true">
    /// Target of the animation
    /// </param>
    ...
    $HGA.SelectionAnimation.initializeBase(this, [target, duration, fps, animations]);
...
$HGA.SelectionAnimation.prototype = {    
    ...
    onStart : function() {
        ...
        /// </summary>
        /// <returns />
	    $HGA.SelectionAnimation.callBaseMethod(this, 'onStart');
    ...
    onEnd : function() {
	    ...
	    $HGA.SelectionAnimation.callBaseMethod(this, 'onEnd');
    }
}
$HGA.SelectionAnimation.registerClass($HGRootNSName + '.Animation.SelectionAnimation', $HGA.ParentAnimation);
$HGA.registerAnimation('selection', $HGA.SelectionAnimation);


$HGA.ConditionAnimation = function(target, duration, fps, animations, conditionScript) {
    /// <summary>
    ...
    $HGA.ConditionAnimation.initializeBase(this, [target, duration, fps, animations]);
    
    // Condition to determine which index we will play
    this._conditionScript = conditionScript;   
}
$HGA.ConditionAnimation.prototype = {    
   getSelectedIndex : function() {
       /// <summary>
       /// Get the index of the animation that is selected to be played.  If this returns an index outside the bounds of
       /// the child animations array, then nothing is played.
...
$HGA.ConditionAnimation.registerClass($HGRootNSName + '.Animation.ConditionAnimation', $HGA.SelectionAnimation);
$HGA.registerAnimation('condition', $HGA.ConditionAnimation);


$HGA.CaseAnimation = function(target, duration, fps, animations, selectScript) {
    /// <summary>
    ...
    $HGA.CaseAnimation.initializeBase(this, [target, duration, fps, animations]);

    // Condition to determine which index we will play
    this._selectScript = selectScript;
}
$HGA.CaseAnimation.prototype = {
    getSelectedIndex : function() {
        /// <summary>
        /// Get the index of the animation that is selected to be played.  If this returns an index outside the bounds of
        /// the child animations array, then nothing is played.
...
$HGA.CaseAnimation.registerClass($HGRootNSName + '.Animation.CaseAnimation', $HGA.SelectionAnimation);
$HGA.registerAnimation('case', $HGA.CaseAnimation);


$HGA.FadeEffect = function() {
    /// <summary>
...
$HGA.FadeEffect.prototype = {
    FadeIn : 0,
    FadeOut : 1
}
$HGA.FadeEffect.registerEnum($HGRootNSName + ".Animation.FadeEffect", false);


$HGA.FadeAnimation = function(target, duration, fps, effect, minimumOpacity, maximumOpacity, forceLayoutInIE) {
    /// <summary>
    /// The <code>FadeAnimation</code> is used to fade an element in or out of view, depending on the
    /// provided <see cref=$HBRootNSName + ".Animation.FadeEffect" />, by settings its opacity.
    /// The minimum and maximum opacity values can be specified to precisely control the fade.
    /// You may also consider using <see cref=$HBRootNSName + ".Animation.FadeInAnimation" /> or
    /// <see cref=$HBRootNSName + ".Animation.FadeOutAnimation" /> if you know the only direction you
    /// are fading.
    /// </summary>
    /// <param name="target" type="Sys.UI.DomElement" mayBeNull="true" optional="true" domElement="true">
    /// Target of the animation
    ...
    /// Number of steps per second.  The default is 25.
    /// </param>
    /// <param name="effect" type=$HBRootNSName + ".Animation.FadeEffect" mayBeNull="true" optional="true">
    ...
    $HGA.FadeAnimation.initializeBase(this, [target, duration, fps]);

    // The effect determines whether or not we fade in or out
    this._effect = (effect !== undefined) ? effect : $HGA.FadeEffect.FadeIn;
...
$HGA.FadeAnimation.prototype = {
    _resetOpacities : function() {
        ...
        if (this._effect == $HGA.FadeEffect.FadeIn) {
            this._start = this._min;
            this._end = this._max;
        } else {
    ...
    onStart : function() {
        ...
        /// </summary>
        /// <returns />       
        $HGA.FadeAnimation.callBaseMethod(this, 'onStart');
...
//        /// <remarks>
//        $HGA.FadeAnimation.callBaseMethod(this, 'set_target', [value]);
//        
//        var element = value;
//        if (element) {
...
//    },
    ...
    get_effect : function() {
    	/// <value type=$HBRootNSName + ".Animation.FadeEffect">
    ...
    set_effect : function(value) {
        value = this._getEnum(value, $HGA.FadeEffect);
...
$HGA.FadeAnimation.registerClass($HGRootNSName + '.Animation.FadeAnimation', $HGA.Animation);
$HGA.registerAnimation('fade', $HGA.FadeAnimation);


$HGA.FadeInAnimation = function(target, duration, fps, minimumOpacity, maximumOpacity, forceLayoutInIE) {
    /// <summary>
    ...
    $HGA.FadeInAnimation.initializeBase(this, [target, duration, fps, $HGA.FadeEffect.FadeIn, minimumOpacity, maximumOpacity, forceLayoutInIE]);
}
$HGA.FadeInAnimation.prototype = {
    onStart : function() {
    	/// <summary>
        /// The <code>onStart</code> method is called just before the animation is played each time.
        /// </summary>
        /// <returns />
        $HGA.FadeInAnimation.callBaseMethod(this, 'onStart');
...
$HGA.FadeInAnimation.registerClass($HGRootNSName + '.Animation.FadeInAnimation', $HGA.FadeAnimation);
$HGA.registerAnimation('fadeIn', $HGA.FadeInAnimation);


$HGA.FadeOutAnimation = function(target, duration, fps, minimumOpacity, maximumOpacity, forceLayoutInIE) {
    /// <summary>
    ...
    $HGA.FadeOutAnimation.initializeBase(this, [target, duration, fps, $HGA.FadeEffect.FadeOut, minimumOpacity, maximumOpacity, forceLayoutInIE]);
}
$HGA.FadeOutAnimation.prototype = {
    onStart : function() {
    	/// <summary>
        /// The <code>onStart</code> method is called just before the animation is played each time.
        /// </summary>
        /// <returns />
        $HGA.FadeOutAnimation.callBaseMethod(this, 'onStart');
...
$HGA.FadeOutAnimation.registerClass($HGRootNSName + '.Animation.FadeOutAnimation', $HGA.FadeAnimation);
$HGA.registerAnimation('fadeOut', $HGA.FadeOutAnimation);


$HGA.PulseAnimation = function(target, duration, fps, iterations, minimumOpacity, maximumOpacity, forceLayoutInIE) {
    /// <summary>
    ...
    $HGA.PulseAnimation.initializeBase(this, [target, duration, fps, null, ((iterations !== undefined) ? iterations : 3)]);

    // Create the FadeOutAnimation
    this._out = new $HGA.FadeOutAnimation(target, duration, fps, minimumOpacity, maximumOpacity, forceLayoutInIE);
    this.add(this._out);
    
    // Create the FadeInAnimation
    this._in = new $HGA.FadeInAnimation(target, duration, fps, minimumOpacity, maximumOpacity, forceLayoutInIE);
    this.add(this._in);
}
$HGA.PulseAnimation.prototype = {
    ...
    set_duration : function(value) {
        ...
        /// </value>
        value = this._getFloat(value);
        $HGA.PulseAnimation.callBaseMethod(this, 'set_duration', [value]);
    ...
    set_fps : function(value) {
        ...
        /// </value>
        value = this._getInteger(value);
        $HGA.PulseAnimation.callBaseMethod(this, 'set_fps', [value]);
        this._in.set_fps(value);
        this._out.set_fps(value);
    }
    
}
$HGA.PulseAnimation.registerClass($HGRootNSName + '.Animation.PulseAnimation', $HGA.SequenceAnimation);
$HGA.registerAnimation('pulse', $HGA.PulseAnimation);


$HGA.PropertyAnimation = function(target, duration, fps, property, propertyKey) {
    /// <summary>
    ...
    $HGA.PropertyAnimation.initializeBase(this, [target, duration, fps]);
...
$HGA.PropertyAnimation.prototype = {
    onStart : function() {
    	/// <summary>
        /// The <code>onStart</code> method is called just before the animation is played each time.
        /// </summary>
        /// <returns />
        $HGA.PropertyAnimation.callBaseMethod(this, 'onStart');
...
$HGA.PropertyAnimation.registerClass($HGRootNSName + '.Animation.PropertyAnimation', $HGA.Animation);
$HGA.registerAnimation('property', $HGA.PropertyAnimation);


$HGA.DiscreteAnimation = function(target, duration, fps, property, propertyKey, values) {
    /// <summary>
    /// The <code>DiscreteAnimation</code> inherits from <see cref=$HBRootNSName + ".Animation.PropertyAnimation" />
    ...
    $HGA.DiscreteAnimation.initializeBase(this, [target, duration, fps, property, propertyKey]);

    // Values to assign to the property
    this._values = (values && values.length) ? values : [];
}
$HGA.DiscreteAnimation.prototype = {
    getAnimatedValue : function(percentage) {
        /// <summary>
...
$HGA.DiscreteAnimation.registerClass($HGRootNSName + '.Animation.DiscreteAnimation', $HGA.PropertyAnimation);
$HGA.registerAnimation('discrete', $HGA.DiscreteAnimation);


$HGA.InterpolatedAnimation = function(target, duration, fps, property, propertyKey, startValue, endValue) {
    /// <summary>
    ...
    $HGA.InterpolatedAnimation.initializeBase(this, [target, duration, fps, ((property !== undefined) ? property : 'style'), propertyKey]);

    // Start and end values
    this._startValue = startValue;
    this._endValue = endValue;
}
$HGA.InterpolatedAnimation.prototype = {
...
$HGA.InterpolatedAnimation.registerClass($HGRootNSName + '.Animation.InterpolatedAnimation', $HGA.PropertyAnimation);
$HGA.registerAnimation('interpolated', $HGA.InterpolatedAnimation);


$HGA.ColorAnimation = function(target, duration, fps, property, propertyKey, startValue, endValue) {
    /// <summary>
    ...
    $HGA.ColorAnimation.initializeBase(this, [target, duration, fps, property, propertyKey, startValue, endValue]);
...
$HGA.ColorAnimation.prototype = {
    onStart : function() {
        /// <summary>
        /// Determine which dimensions of color will be animated
        /// </summary>
        /// <returns />
        $HGA.ColorAnimation.callBaseMethod(this, 'onStart');
       
        this._start = $HGA.ColorAnimation.getRGB(this.get_startValue());
        this._end = $HGA.ColorAnimation.getRGB(this.get_endValue());
    ...
    getAnimatedValue : function(percentage) {
        ...
        return $HGA.ColorAnimation.toColor(r, g, b);
    },
...
$HGA.ColorAnimation.getRGB = function(color) {
    /// <summary>
...
$HGA.ColorAnimation.toColor = function(red, green, blue) {
    /// <summary>
...
$HGA.ColorAnimation.registerClass($HGRootNSName + '.Animation.ColorAnimation', $HGA.InterpolatedAnimation);
$HGA.registerAnimation('color', $HGA.ColorAnimation);


$HGA.LengthAnimation = function(target, duration, fps, property, propertyKey, startValue, endValue, unit) {
    /// <summary>
    /// The <code>LengthAnimation</code> is identical to <see cref=$HBRootNSName + ".Animation.InterpolatedAnimation" />
    ...
    $HGA.LengthAnimation.initializeBase(this, [target, duration, fps, property, propertyKey, startValue, endValue]);
    
    // Unit of length (which defaults to px)
    this._unit = (unit != null) ? unit : 'px';
}
$HGA.LengthAnimation.prototype = {
...
$HGA.LengthAnimation.registerClass($HGRootNSName + '.Animation.LengthAnimation', $HGA.InterpolatedAnimation);
$HGA.registerAnimation('length', $HGA.LengthAnimation);


$HGA.MoveAnimation = function(target, duration, fps, horizontal, vertical, relative, unit) {
    /// <summary>
    ...
    $HGA.MoveAnimation.initializeBase(this, [target, duration, fps, null]);
    ...
    this._horizontalAnimation = new $HGA.LengthAnimation(target, duration, fps, 'style', 'left', null, null, unit);
    this._verticalAnimation = new $HGA.LengthAnimation(target, duration, fps, 'style', 'top', null, null, unit);
    this.add(this._verticalAnimation);
    this.add(this._horizontalAnimation);
}
$HGA.MoveAnimation.prototype = {
    ...
    onStart : function() {
        ...
        /// </summary>
        /// <returns />
        $HGA.MoveAnimation.callBaseMethod(this, 'onStart');
...
$HGA.MoveAnimation.registerClass($HGRootNSName + '.Animation.MoveAnimation', $HGA.ParallelAnimation);
$HGA.registerAnimation('move', $HGA.MoveAnimation);


$HGA.ResizeAnimation = function(target, duration, fps, width, height, unit) {
    /// <summary>
    ...
    $HGA.ResizeAnimation.initializeBase(this, [target, duration, fps, null]);
    ...
    this._horizontalAnimation = new $HGA.LengthAnimation(target, duration, fps, 'style', 'width', null, null, unit);
    this._verticalAnimation = new $HGA.LengthAnimation(target, duration, fps, 'style', 'height', null, null, unit);
    this.add(this._horizontalAnimation);
    this.add(this._verticalAnimation);
}
$HGA.ResizeAnimation.prototype = {
    ...
    onStart : function() {
        ...
        $HGA.ResizeAnimation.callBaseMethod(this, 'onStart');
...
$HGA.ResizeAnimation.registerClass($HGRootNSName + '.Animation.ResizeAnimation', $HGA.ParallelAnimation);
$HGA.registerAnimation('resize', $HGA.ResizeAnimation);
...
$HGA.ScaleAnimation = function(target, duration, fps, scaleFactor, unit, center, scaleFont, fontUnit) {
    /// <summary>
    ...
    $HGA.ScaleAnimation.initializeBase(this, [target, duration, fps]);
...
$HGA.ScaleAnimation.prototype = {    
    ...
    onStart : function() {
        ...
        /// </summary>
        /// <returns />
        $HGA.ScaleAnimation.callBaseMethod(this, 'onStart');
    ...
    onEnd : function() {
        ...
        this._initialLeft = null;
        this._initialFontSize = null;
        $HGA.ScaleAnimation.callBaseMethod(this, 'onEnd');
    },
...
$HGA.ScaleAnimation.registerClass($HGRootNSName + '.Animation.ScaleAnimation', $HGA.Animation);
$HGA.registerAnimation('scale', $HGA.ScaleAnimation);


$HGA.Action = function(target, duration, fps) {
    /// <summary>
    ...
    $HGA.Action.initializeBase(this, [target, duration, fps]);
...
$HGA.Action.prototype = {
    ...
    onEnd : function() {
        ...
        /// <returns />
        this.doAction();
        $HGA.Action.callBaseMethod(this, 'onEnd');
    },
...
$HGA.Action.registerClass($HGRootNSName + '.Animation.Action', $HGA.Animation);
$HGA.registerAnimation('action', $HGA.Action);


$HGA.EnableAction = function(target, duration, fps, enabled) {
    /// <summary>
    ...
    $HGA.EnableAction.initializeBase(this, [target, duration, fps]);

    // Whether to enable or disable
    this._enabled = (enabled !== undefined) ? enabled : true;
}
$HGA.EnableAction.prototype = {
    doAction : function() {
    	/// <summary>
...
$HGA.EnableAction.registerClass($HGRootNSName + '.Animation.EnableAction', $HGA.Action);
$HGA.registerAnimation('enableAction', $HGA.EnableAction);


$HGA.HideAction = function(target, duration, fps) {
    /// <summary>
    ...
    $HGA.HideAction.initializeBase(this, [target, duration, fps]);

}
$HGA.HideAction.prototype = {
    doAction : function() {
        /// <summary>
...
$HGA.HideAction.registerClass($HGRootNSName + '.Animation.HideAction', $HGA.Action);
$HGA.registerAnimation('hideAction', $HGA.HideAction);


$HGA.StyleAction = function(target, duration, fps, attribute, value) {
    /// <summary>
    ...
    $HGA.StyleAction.initializeBase(this, [target, duration, fps]);
...
$HGA.StyleAction.registerClass($HGRootNSName + '.Animation.StyleAction', $HGA.Action);
$HGA.registerAnimation('styleAction', $HGA.StyleAction);


$HGA.OpacityAction = function(target, duration, fps, opacity) {
    /// <summary>
    ...
    $HGA.OpacityAction.initializeBase(this, [target, duration, fps]);
    
    // Opacity
    this._opacity = opacity;
}
$HGA.OpacityAction.prototype = {
    doAction : function() {
    	/// <summary>
        /// Set the opacity
    	/// </summary>
...
$HGA.OpacityAction.registerClass($HGRootNSName + '.Animation.OpacityAction', $HGA.Action);
$HGA.registerAnimation('opacityAction', $HGA.OpacityAction);


$HGA.ScriptAction = function(target, duration, fps, script) {
    /// <summary>
    ...
    $HGA.ScriptAction.initializeBase(this, [target, duration, fps]);

    // Script to execute
    this._script = script;
}
$HGA.ScriptAction.prototype = {
    doAction : function() {
    	/// <summary>
        /// Execute the script
    	/// </summary>
...
$HGA.ScriptAction.registerClass($HGRootNSName + '.Animation.ScriptAction', $HGA.Action);
$HGA.registerAnimation('scriptAction', $HGA.ScriptAction);