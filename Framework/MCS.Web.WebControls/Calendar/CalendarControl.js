...
// -------------------------------------------------
// FileName	：	CalendarControl.js
// Remark	：	
// -------------------------------------------------
// VERSION  	AUTHOR		DATE			CONTENT
// 
// -------------------------------------------------
//var $HBRootNSName = 'ChinaCustoms.Framework.DeluxeWorks.Web.Controls';
//Type.registerNamespace($HBRootNSName);
//var $HBRootNS = eval($HBRootNSName); 

$HGRootNS.CalendarControl = function(element) {
 /// <summary>
...
;
    ...
    $HGRootNS.CalendarControl.initializeBase(this, [element]);
...
$HGRootNS.CalendarControl.prototype = {    
    ...
    initialize : function() {
        ...
        $HGRootNS.CalendarControl.callBaseMethod(this, "initialize");
...
//            }
    ...
    dispose : function() {
        ...
        if (this._titleselect){
           $HGCommon.removeHandlers(this._titleselect, this._select$delegates);
            this._titleselect = null;
        }
        if (this._titleimg){
           $HGCommon.removeHandlers(this._titleimg, this._img$delegates);
            this._titleimg = null;
        }
        //
        if (this._popupDiv) {
            $HGCommon.removeHandlers(this._popupDiv, this._popup$delegates);
            this._popupDiv = null;
        }        
        if (this._prevArrow) {
            $HGCommon.removeHandlers(this._prevArrow, this._cell$delegates);
            this._prevArrow = null;
        }
        if (this._prevArrowImage) {
            $HGCommon.removeHandlers(this._prevArrowImage, this._cell$delegates);
            this._prevArrowImage = null;
        }
        if (this._nextArrow) {
            $HGCommon.removeHandlers(this._nextArrow, this._cell$delegates);
            this._nextArrow = null;
        }
        if (this._nextArrowImage) {        
            $HGCommon.removeHandlers(this._nextArrowImage, this._cell$delegates);
            this._nextArrowImage = null;
        }
        if (this._title) {
            $HGCommon.removeHandlers(this._title, this._cell$delegates);
            this._title = null;
        }
        if (this._today) {
            $HGCommon.removeHandlers(this._today, this._cell$delegates);
            this._today = null;
        }
        if (this._daysRow) {
            for (var i = 0; i < this._daysBody.rows.length; i++) {
                ...
                for (var j = 0; j < row.cells.length; j++) {
                    $HGCommon.removeHandlers(row.cells[j].firstChild, this._cell$delegates);
                }
            }
            this._daysRow = null;
        }
        ...
        var elt = this.get_element();
        $HGCommon.removeHandlers(elt, this._element$delegates);
        $HGRootNS.CalendarControl.callBaseMethod(this, "dispose");
    },
    ...
    _buildCalendar : function() {
        ...
        this._container = $HGCommon.createElementFromTemplate({
            nodeName : "div",
         ...
         this._popupDiv = $HGCommon.createElementFromTemplate({ 
            nodeName : "div",
    ...
    _buildHeader : function() {
        ...
        this._header = $HGCommon.createElementFromTemplate({ 
            nodeName : "div",
        ...
        {
        var prevArrowWrapper = $HGCommon.createElementFromTemplate({ nodeName : "div" }, this._header);
        this._prevArrow = $HGCommon.createElementFromTemplate({ 
            nodeName : "div",
        ...
        var nextArrowWrapper = $HGCommon.createElementFromTemplate({ nodeName : "div" }, this._header);
        this._nextArrow = $HGCommon.createElementFromTemplate({ 
            nodeName : "div",
        ...
        {
                    /*0508*/
        var titlePadding = $HGCommon.createElementFromTemplate({ nodeName : "div" }, this._header);        
        this._padding = $HGCommon.createElementFromTemplate({ 
            nodeName : "div",
            cssClasses : [ "ajax__calendar_padding" ] 
        }, titlePadding);      
        //add list
        
        this._selectMonthList = $HGCommon.createElementFromTemplate({ nodeName : "select" , properties : {
//                tabIndex : 0
            },events : this._select$delegates,cssClasses : [ "ajax__calendar_titleselect" ] }, this._header);
       
        for(var scount=0;scount<12;scount++)
        {
           _titleselect = $HGCommon.createElementFromTemplate({
    ...
    var clickYearImg = $HGCommon.createElementFromTemplate({ nodeName : "div" }, this._header);
       this._titleimg = $HGCommon.createElementFromTemplate({
            nodeName : "div", properties : { mode : "next" },
            events : this._img$delegates,cssClasses : [ "ajax__calendar_titleimg" ]  }, clickYearImg);
            
            var clickYearInput = $HGCommon.createElementFromTemplate({ nodeName : "div" , properties : {
//                tabIndex : 0
            },events : this._click$delegates}, this._header);
       this._titleinput = $HGCommon.createElementFromTemplate({
            nodeName : "input",
            events : this._cell$delegates,cssClasses : [ "ajax__calendar_titleinput" ]  }, clickYearInput);
     }
        var titleWrapper = $HGCommon.createElementFromTemplate({ nodeName : "div" }, this._header);        
        this._title = $HGCommon.createElementFromTemplate({ 
            nodeName : "div",
    ...
    _buildBody : function() {
        ...
        this._body = $HGCommon.createElementFromTemplate({ 
            nodeName : "div",
    ...
    _buildFooter : function() {
        ...
        var todayWrapper = $HGCommon.createElementFromTemplate({ nodeName : "div" }, this._popupDiv);
        this._today = $HGCommon.createElementFromTemplate({
            nodeName : "div",
    ...
    _buildDays : function() {
        ...
        this._days = $HGCommon.createElementFromTemplate({ 
            nodeName : "div",
            cssClasses : [ "ajax__calendar_days" ]
        }, this._body);
        this._modes["days"] = this._days;
        
        this._daysTable = $HGCommon.createElementFromTemplate({ 
            nodeName : "table",
            properties : {
                cellPadding : 0,
                cellSpacing : 0,
        ...
        this._daysTableHeader = $HGCommon.createElementFromTemplate({ nodeName : "thead" }, this._daysTable);
        this._daysTableHeaderRow = $HGCommon.createElementFromTemplate({ nodeName : "tr" }, this._daysTableHeader);
        this._daysBody = $HGCommon.createElementFromTemplate({ nodeName: "tbody" }, this._daysTable);
        
        for (var i = 0; i < 7; i++) {
            var dayCell = $HGCommon.createElementFromTemplate({ nodeName : "td" }, this._daysTableHeaderRow);
            var dayDiv = $HGCommon.createElementFromTemplate({
                nodeName : "div",
        ...
        for (var i = 0; i < 6; i++) {
            var daysRow = $HGCommon.createElementFromTemplate({ nodeName : "tr" }, this._daysBody);
            for(var j = 0; j < 7; j++) {
                var dayCell = $HGCommon.createElementFromTemplate({ nodeName : "td" }, daysRow);
                var dayDiv = $HGCommon.createElementFromTemplate({
                    nodeName : "div",
                    properties : {
                        mode : "day",
    ...
    _buildMonths : function() {
        ...
        this._months = $HGCommon.createElementFromTemplate({
            nodeName : "div",
        ...
        this._monthsTable = $HGCommon.createElementFromTemplate({
            nodeName : "table",
            properties : {
                cellPadding : 0,
                cellSpacing : 0,
        ...
        this._monthsBody = $HGCommon.createElementFromTemplate({ nodeName : "tbody" }, this._monthsTable);
        for (var i = 0; i < 3; i++) {
            var monthsRow = $HGCommon.createElementFromTemplate({ nodeName : "tr" }, this._monthsBody);
            for (var j = 0; j < 4; j++) {
                var monthCell = $HGCommon.createElementFromTemplate({ nodeName : "td" }, monthsRow);
                var monthDiv = $HGCommon.createElementFromTemplate({
                    nodeName : "div",
                    properties : {
                        mode : "month",
                        month : (i * 4) + j,
    ...
    _buildYears : function() {
        ...
        this._years = $HGCommon.createElementFromTemplate({
            nodeName : "div",
        ...
        this._yearsTable = $HGCommon.createElementFromTemplate({
            nodeName : "table",
            properties : {
                cellPadding : 0,
                cellSpacing : 0,
        ...
        this._yearsBody = $HGCommon.createElementFromTemplate({ nodeName : "tbody" }, this._yearsTable);        
        for (var i = 0; i < 3; i++) {
            var yearsRow = $HGCommon.createElementFromTemplate({ nodeName : "tr" }, this._yearsBody);
            for (var j = 0; j < 4; j++) {
                var yearCell = $HGCommon.createElementFromTemplate({ nodeName : "td" }, yearsRow);
                var yearDiv = $HGCommon.createElementFromTemplate({ 
                    nodeName : "div", 
                    properties : { 
                        mode : "year",
                        year : ((i * 4) + j) - 1
                ...
                for (var week = 0; week < 6; week ++) {
                    ...
                    for (var dayOfWeek = 0; dayOfWeek < 7; dayOfWeek++) {
                        ...
                        if(this._isOnlyCurrentMonth)
                        {
                           ...
                           $HGCommon.removeCssClasses(dayCell.parentNode, [ "ajax__calendar_other", "ajax__calendar_active"]);
                           $HGCommon.removeCssClasses(dayCell.parentNode, [ "ajax__calendar_currentmonthday"]);
            ...
            case "months":
                ...
                for (var i = 0; i < this._monthsBody.rows.length; i++) {
                    ...
                    for (var j = 0; j < row.cells.length; j++) {
                        ...
                        $HGCommon.removeCssClasses(cell.parentNode, [ "ajax__calendar_other", "ajax__calendar_active" ]);
...
//            var doc = $HGCommon.get_currentDocument;
            $HGCommon.set_currentDocument(this._calendarPopup.get_popupDocument());
//            try
//            {
                //figure
                this._buildCalendar();
                ...
                this._buildBody();
                this._buildFooter();
                $HGCommon.set_currentDocument(document);
//            }
//            finally
//            {
//                $HGCommon.set_currentDocument(doc);
//            }
        }    
    },
    ...
    _switchMonth : function(date, dontAnimate) {
        ...
        if (this._animated && !dontAnimate) {
            ...
            if (visibleDate > date) {
                ...
                // move in, fade in
                $HGCommon.setLocation(newElement, {x:-162,y:0});
                ...
                // move out, fade out
                $HGCommon.setLocation(oldElement, {x:0,y:0});
    ...
    _switchMode : function(mode, dontAnimate) {
        ...
        if (this._animated && !dontAnimate) { 
            ...
            if (moveDown) {   
                ...
                // move in, fade in
                $HGCommon.setLocation(newElement, {x:0,y:-this._height});
                Sys.UI.DomElement.setVisible(newElement, true);
                ...
                // move in, fade in
                $HGCommon.setLocation(newElement, {x:0,y:139});
                Sys.UI.DomElement.setVisible(newElement, true);
                this._modeChangeMoveBottomOrRightAnimation.set_propertyKey("top");
                this._modeChangeMoveBottomOrRightAnimation.set_target(newElement);
                this._modeChangeMoveBottomOrRightAnimation.set_endValue(0);
            ...
            this.invalidate();
            Sys.UI.DomElement.setVisible(newElement, true);
            $HGCommon.setLocation(newElement, {x:0,y:0});
        }
    },
...
$HGRootNS.CalendarControl.registerClass($HGRootNSName + ".CalendarControl", $HGRootNS.ControlBase);
