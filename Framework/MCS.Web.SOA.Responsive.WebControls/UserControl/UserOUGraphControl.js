...
$HBRootNS.UserOUGraphControl.prototype =
{
        ...
        if (value) {
            if (value != "") {
                ...
                if (this._treeControlID) {
                    ...
                    this._tree._dataBind(this._rootNodesData);
                }
            }
        }
    },
    ...
    _onConfirm: function (args) {
        var tree = $find(this._treeControlID);
        var multiSelect = this.get_multiSelect(); // (event.srcElement.multiSelect == "true");
        ...
        if (result.length <= 0)//如果没有选择人员，按下确定。提示用户并返回。
        {
            alert($NT.getText("SOAWebControls", "请选择机构或人员。"));
            args.canceled = true;
            return;
        }

        args.result = Sys.Serialization.JavaScriptSerializer.serialize(result);
    },

    _onCancel: function () { },

    dataBind: function (args) {
        this._dialogResult = Sys.Serialization.JavaScriptSerializer.deserialize(args.ugcSelectedObjStr);
    ...
    showDialog: function () {
        var params = { ugcSelectedObjStr: Sys.Serialization.JavaScriptSerializer.serialize(this._dialogResult) };
        this._showDialog(this._dialogUrl, params, Function.createDelegate(this, this.onConfirmCallBack));
    },

    onConfirmCallBack: function (sender, args) {
        this._dialogResult = Sys.Serialization.JavaScriptSerializer.deserialize(args.result);
        this.raiseDialogConfirmed();
    },
    ...
    _onTreeAfterDataBind: function (sender, e) {
        var obj = e.node.get_extendedData(), parent;
        ...
        if (obj.objectType == $HBRootNS.UserControlObjectMask.User
            ...
            parent = e.node._nodeImg.parentElement;
            if (parent) {
