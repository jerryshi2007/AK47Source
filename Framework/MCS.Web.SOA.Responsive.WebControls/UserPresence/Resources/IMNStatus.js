...
function NotifyIMNOnStatusChange(name, state, id) {
...
function IMNShowOOUI(inputType) {
	if (browseris.ie5up && browseris.win32) {
		...
		if (EnsureIMNControl() && IMNNameDictionaryObj) {
			...
			var innerWin = IMNGetInnerWindow();

			if (innerWin != null) {
				innerWin.OuterProcessIMNCommand(IMNCreateCommandString({ commandName: "showOOUI", name: name, inputType: inputType, oouiX: oouiX, oouiY: oouiY }));
			}

			//innerDoc.getElementById("monitorAddresses").value = IMNCreateCommandString({ commandName: "showOOUI", name: name, inputType: inputType, oouiX: oouiX, oouiY: oouiY });
		}
	}
}
...
function IMNHideOOUI() {
	...
	if (innerWin != null) {
		innerWin.OuterProcessIMNCommand(IMNCreateCommandString({ commandName: "hideOOUI" }));
	}

//	var innerDoc = IMNGetInnerDoc();

//	if (innerDoc != null)
//		innerDoc.getElementById("monitorAddresses").value = IMNCreateCommandString({ commandName: "hideOOUI" });
...
function ChangeImgToImnElement(img, sipAddress) {
	img.name = "imnmark";
	if (img.setAttribute)
		img.setAttribute("data-sip", sipAddress);
	else
		img["data-sip"] = sipAddress;
...
function ProcessImnMarkers(imnElements) {
	...
	for (var i = 0; i < batchSize; ++i) {
		...
		if (imnElem.getAttribute)
			sip = imnElem.getAttribute("data-sip");
		else
			sip = imnElem['data-sip'];
		IMNRC(sip, imnElem);
		count++;
	}
...
//		if (innerDoc != null)
//			innerDoc.getElementById("monitorAddresses").value = IMNCreateCommandString({ commandName: "subscribe", name: name, id: id });

		var innerWin = IMNGetInnerWindow();

		if (innerWin != null) {
			innerWin.OuterProcessIMNCommand(IMNCreateCommandString({ commandName: "subscribe", name: name, id: id }));
		}
	}
}
...
function IMNGetInnerWindow() {
	var innerWin = null;

	if (document.getElementById("userPresenceInnerPage") != null)
		innerWin = document.getElementById("userPresenceInnerPage").contentWindow;
