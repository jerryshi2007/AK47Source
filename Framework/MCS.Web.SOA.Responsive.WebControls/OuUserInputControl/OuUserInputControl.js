...
$HBRootNS.OuUserInputControl.prototype =
{
    ...
    _beforeCreateItemSpanWithID: function (obj, container) {
        if (this.get_enableUserPresence() && typeof (obj.clientContext) !== 'undefined') {
    ...
    _transactDialogData: function (sender) {
        var i, j, objs = sender.get_selectedObjects(); //得到在对话框中勾选的组织机构人员

        var thisControl = $get(this._inputAreaClientID);
        //像输入框中拼文本信息，如果现有信息最后没有分号则加入一个分号
        var txt;
        if ("textContent" in thisControl) {
            txt = thisControl.textContent.trim();
            if (txt.length) {
                if (txt.substr(txt.length - 1, 1) != ";") {
                    thisControl.textContent = txt + ';';
                }
            }
        } else {
            txt = (thisControl.innerText).trim();
            if (txt.length) {
                if (txt.substr(txt.length - 1, 1) != ";") {
                    thisControl.innerText = txt + ';';
                }
            }
        }
        ...
        for (i = 0; i < this.get_selectedOuUserData().length; i++) {
            var flag = true;
    ...
    _fillItemSpanAttributes: function (obj, aSpan) {
        ...
        if ("textContent" in aSpan)
            aSpan.textContent = obj.displayName + ";";
        else
            aSpan.innerText = obj.displayName + ";";
    ...
    checkData: function () {
        ...
        var thisControl = $get(this._inputAreaClientID);
        var sTmpText = thisControl.textContent || thisControl.innerText;
        ...
        for (n = spanInput.firstChild; n; n = n.nextSibling) {
            if (n.nodeType === 1 && (!n.getAttribute('data-id'))) { // todo: 原来是判断ID属性
                rst = false;
                n = null;
                break;
            }
    ...
    saveClientState: function () {
        ...
        fsCS[1] = spanInput.textContent || spanInput.innerText;
