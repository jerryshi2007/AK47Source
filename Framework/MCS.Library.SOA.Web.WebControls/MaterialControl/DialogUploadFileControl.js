// -------------------------------------------------
// FileName	：	DialogUploadFileControl.js
// Remark	：	批量上传文件
// -------------------------------------------------
...
$HBRootNS.DialogUploadFileControl = function (element) {
    $HBRootNS.DialogUploadFileControl.initializeBase(this, [element]);
    ...
    this._componentHelperActiveX = null;
    ...
    this._selectFileButtonEvents =
		{
		    click: Function.createDelegate(this, this._onSelectFileButtonClick)
		};
    this._removeFileButtonEvents =
		{
		    click: Function.createDelegate(this, this._onRemoveFileButtonClick)
		};
    ...
    this._onSelectFileInputEvents =
        {
            change: Function.createDelegate(this, this._onSelectFileChange)
        };
}

$HBRootNS.DialogUploadFileControl.prototype =
{
    initialize: function () {
    ...
    dispose: function () {
        ...
        $HGDomEvent.removeHandlers(this._selectFileButton, this._selectFileButtonEvents);
        $HGDomEvent.removeHandlers(this._removeFileButton, this._removeFileButtonEvents);
        ...
        $HBRootNS.DialogUploadFileControl.callBaseMethod(this, 'dispose');
    },
    ...
    _setControlDisabled: function () {
        if (this._fileCountLimited != 0 && this._fileCountLimited <= this._materials.length) {
            this._fileList.disabled = true;
            this._selectFileButton.disabled = true;
            this._removeFileButton.disabled = true;
            this._uploadFileImageButton.disabled = true;
        }
        else {
            this._fileList.disabled = false;
            this._selectFileButton.disabled = false;
            this._removeFileButton.disabled = false;
    ...
    _showMainTitle: function () {
        ...
        if (this._fileSelectMode == $HBRootNS.fileSelectMode.TraditionalSingle) {
            ...
            this._createActXObject(tableCell);
        }
        ...
        $HGDomElement.createTextNode(" " + $NT.getText("SOAWebControls", "批量上传文件"), tableCell, $HGDomElement.get_currentDocument());
    },
    ...
    _createFileInput: function (tableCell) {
        this._fileInput = $HGDomElement.createElementFromTemplate(
			{
			    ...
			    events: this._onSelectFileInputEvents
			},
			tableCell
			);
    },
    ...
    _showSelectFiles: function () {
        ...
        tableCell.innerHTML = "<b>" + $NT.getText("SOAWebControls", "已选择文件列表") + "</b>";

        tableRow = this._buildTableRow(this._tableMain);
        tableCell = this._buildTableCell(tableRow, 1);

        this._fileList = $HGDomElement.createElementFromTemplate(
			{
			    nodeName: "select",
			    properties:
				{
    ...
    _showSelectFileButton: function (tableButton) {
        var tableRow = this._buildTableRow(tableButton);
        var tableCell = this._buildTableCell(tableRow);
        tableCell.vAlign = "top";

        this._selectFileButton = $HGDomElement.createElementFromTemplate(
			{
			    nodeName: "input",
			    properties:
				{
				    type: "button",
				    value: $NT.getText("SOAWebControls", "选择文件(S)"),
				    accessKey: "S",
				    style:
					{
					    fontSize: "10px",
			    ...
			    cssClasses: ["formButton"],
			    events: this._selectFileButtonEvents
			},
			tableCell
			);
    },

    _showRemoveFileButton: function (tableButton) {
        var tableRow = this._buildTableRow(tableButton);
        var tableCell = this._buildTableCell(tableRow);

        this._removeFileButton = $HGDomElement.createElementFromTemplate(
			{
			    nodeName: "input",
			    properties:
				{
				    type: "button",
				    value: $NT.getText("SOAWebControls", "移除文件(R)"),
				    accessKey: "R",
				    style:
					{
					    fontSize: "10px",
			    ...
			    cssClasses: ["formButton"],
			    events: this._removeFileButtonEvents
			},
			tableCell
			);
    },
    ...
    _onSelectFileButtonClick: function () {
        if (this._fileSelectMode == $HBRootNS.fileSelectMode.TraditionalSingle) {
            this._fileInput.click();
        }
        else if (this._fileSelectMode == $HBRootNS.fileSelectMode.MultiSelectUseActiveX) {
            try {
                this._actXSelectFile.MultiSelect = true;
                var selectFileDialog = this._actXSelectFile.OpenDialog();
                var item;
                var fileNames;

                if (selectFileDialog) {
                    fileNames = this._actXSelectFile.FileName.split('|');

                    if (fileNames.length == 1 && this._fileContains(fileNames[0]) == false)
                        this._selectOneFile(fileNames[0]);
                    else
                        this._selectFiles(fileNames);

                    this._actXSelectFile.FileName = "";
                }
            }
            catch (e) {
                if (e.description.indexOf("ErrorCode:12291") != -1)
                    alert($NT.getText("SOAWebControls", "您选择的文件路径长度超过了限制，请尝试减少选择文件的数量"));
                else
                    throw Error.create(e.description);
            }
        }
    },

    _selectOneFile: function (fileName) {
        var item = this._fileList.document.createElement("Option");
        item.text = $HBRootNS.fileIO.getFileNameWithExt(fileName);
        item.value = fileName;
        var canUploadFieCount = this._fileCountLimited - this._materials.length;
        if (this._fileCountLimited == 0 || this._fileCountLimited != 0 && this._fileList.length < canUploadFieCount) {
            this._fileList.add(item);
        }
    },

    _selectFiles: function (fileNames) {
        var fileFullPath, item, canUploadFieCount;
        canUploadFieCount = this._fileCountLimited - this._materials.length;

        for (var i = 1; i < fileNames.length; i++) {
            fileFullPath = fileNames[0] + "\\" + fileNames[i];

            if (this._fileContains(fileFullPath) == false) {
                if (this._fileCountLimited == 0 || this._fileCountLimited != 0 && this._fileList.length < canUploadFieCount) {
                    item = this._fileList.document.createElement("Option");
                    item.text = $HBRootNS.fileIO.getFileNameWithExt(fileFullPath);
                    item.value = fileFullPath;
                    this._fileList.add(item);
                }
            }
        }
    ...
    _fileContains: function (fileName) {
        if (this._fileList.length == 0)
            return false;

        for (var i = 0; i < this._fileList.length; i++) {
            if (this._fileList.options[i].value == fileName)
                return true;
        }
        return false;
    },
    ...
    _onUploadFileButtonClick: function () {
        if (this._fileList.length == 0) {
            alert($NT.getText("SOAWebControls", "至少要选择一个上传文件！"));
            return;
        }
        ...
        for (var i = 0; i < this._fileList.length; i++) {
            sampleMaterial = new Object();
            sampleMaterial.filePath = this._fileList[i].value;
            sampleMaterial.container = this._container;
            Array.add(filesToUpload, sampleMaterial);
        }
        ...
        information.container = this._container;
        information.isNew = true;
        //information.componentHelperActiveXID = this._container.get_componentHelperActiveXID();
        var filesUploaded = window.showModalDialog(this._container.get_dialogUploadFileProcessControlUrl(), information, arg);

        if (filesUploaded) {
            for (var i = 0; i < filesUploaded.length; i++) {
                this._addNewMateial(filesUploaded[i]);
            }

            this._removeUploadedFiles(filesUploaded);
            this._showMaterials();
        }
    ...
    _onOkButtonClick: function () {
        if (this._fileList.options.length != 0) {
            if (window.confirm($NT.getText("SOAWebControls", "您还有未上传的文件，确定要关闭本窗口吗？")))
...
$HBRootNS.DialogUploadFileControl.registerClass($HBRootNSName + ".DialogUploadFileControl", $HGRootNS.ControlBase);
