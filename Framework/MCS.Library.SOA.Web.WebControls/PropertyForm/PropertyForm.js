...
$HGRootNS.PropertyForm.prototype =
{
	...
	_get_activePropertyStyleName: function () {
		return "ajax__propertyForm_nameCell_selected";
	},
		...
		var table = $HGDomElement.createElementFromTemplate(
				{
					...
					cssClasses: ["ajax__formGrid_table"]
				},
				this.get_element()
				);
	...
	_renderSectionRows: function (sectionContainer) {
		...
		for (var item in categories) {
			...
			var bodyRow = $HGDomElement.createElementFromTemplate({
				nodeName: "tr"
			}, sectionContainer);

			var bodycell = $HGDomElement.createElementFromTemplate({
				nodeName: "td"
			}, bodyRow);
	...
	_renderSectiontop: function (setopContainer, sectionObject) {
		var cell = $HGDomElement.createElementFromTemplate(
				{
					nodeName: "td",
					cssClasses: ["ajax__propertyForm_header"],
					properties: {
						innerText: sectionObject.displayName
					}
				}, setopContainer
				);
	},
