...
FROM SC.SchemaMembersSnapshot UC
	INNER JOIN SC.SchemaObjectSnapshot SCContainer ON UC.ContainerID = SCContainer.ID 
	INNER JOIN SC.SchemaRelationObjectsSnapshot SRContainer ON UC.ContainerID = SRContainer.ObjectID 
	INNER JOIN SC.SchemaRelationObjectsSnapshot SR ON UC.MemberID = SR.ObjectID INNER JOIN SC.SchemaObjectSnapshot SC ON SC.ID = SR.ObjectID
WHERE {0}