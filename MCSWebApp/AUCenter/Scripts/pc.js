...
(function (window) {
    ...
    pcHelper.modalPopup = function (elem,width,height) {
        // 无模态弹出方式弹出超链接窗口
        if (elem && elem.nodeType === 1 && elem.nodeName.toUpperCase() === "A") {
            var href = '';
            if (elem.getAttribute)
            ...
            width = width || pcHelper.getAttr(elem, "data-pop-width") || 800;
            height = height || pcHelper.getAttr(elem, "data-pop-height") || 600;
            pcHelper.showDialog(href, '', null, false, width, height, true);
            return false;
        } else {
            return true;
        }
