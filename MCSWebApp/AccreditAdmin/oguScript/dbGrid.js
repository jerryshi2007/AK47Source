...
/**************************************************************************************/
// forever Add In 2004-04-14
// 功能：根据XMl数据组织来更新数据行（数据采用XML属性的方式）
// xmlTable ： 包含XML数据的新节点
// oRow ： 要求被更新的数据行
// oHeadRow ： 行中所有数据的格式
// bAddNullRow ：是否对于未设定数据采用空格填充
// callBackFun ：数据回填所需调用的填充函数
// arguments.. ：要求行填充的时候不被填充的列，如：“SORT_ID”
/**************************************************************************************/
function updateGridCellWithAttributes(xmlTable, oRow, oHeadRow, bAddNullRow, callBackFun)
{
	for (var i = 0; i < oHeadRow.childNodes.length; i++)
	{
		var bChange = true;
		
		var oTD = oRow.childNodes[i];
		...
		if (xmlTable != null)
		{
			...
			if (bChange)
				setCellContent(xmlTable, oTD, strFld, callBackFun);
		}
		else
		{
			if (bAddNullRow)
...
function _afterCreateTable(oTable) {
	if (oTable != null) {
...
function createGridBodyByHeadAsync(xmlFirst, oBody, xmlDicts, oHeadRow, oTable, gridRowCallBack, gridAfterCallBack)
{
	...
	var callBackObj = { rowCallBack: gridRowCallBack, innerAllCallBack: _afterCreateTable, allCallBack: gridAfterCallBack };

	_createGridBodyByHeadRepeatly(xmlFirst, oBody, xmlDicts, 20, oHeadRow, oTable, callBackObj);
}

function _createGridBodyByHeadRepeatly(xmlFirst, oBody, xmlDicts, blockSize, oHeadRow, oTable, callBackObj) {
	if (xmlFirst) {
		window.setTimeout(function () {
				xmlFirst = _createRows(xmlFirst, oBody, xmlDicts, blockSize, oHeadRow, callBackObj.rowCallBack);
				_createGridBodyByHeadRepeatly(xmlFirst, oBody, xmlDicts, blockSize, oHeadRow, oTable, callBackObj);
			}, 0);
	}
	else{
		if (callBackObj.innerAllCallBack)
			callBackObj.innerAllCallBack(oTable);

		if (callBackObj.allCallBack)
			callBackObj.allCallBack();
	}
}

function _createRows(xmlFirst, oBody, xmlDicts, count, oHeadRow, callBackFun)
{
	var i = 0;
	while (xmlFirst && i < count)
	{
