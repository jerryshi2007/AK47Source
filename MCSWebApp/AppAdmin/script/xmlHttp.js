...
function createCommandXML(strCommand)
{
	...
	var nodeCommand = xmlData.createNode(1, "COMMAND", "");

	appendAttr(xmlData, nodeCommand, "name", strCommand);

	root.appendChild(nodeCommand);
		...
		if (paramIsArray(arguments[1]) == false)
		{
			for (var i = 1; i < arguments.length; i++)
				if (typeof(arguments[i]) != "undefined")
					appendNode(xmlData, nodeCommand, "PARAM", arguments[i]);
		}
		else
		{
			for (var i = 0; i < arguments[1].length; i++)
				appendNode(xmlData, nodeCommand, "PARAM", arguments[1][i]);
		}
	}
...
function createInsertDoc(collection, tableName, getOperation)
{
	...
	var nodeCommand = xmlData.createNode(1, tableName, "");
	root.appendChild(nodeCommand);
	
	var node = xmlData.createNode(1, "SET", "");
	nodeCommand.appendChild(node);
		
	getInfoFromCollection(xmlData, node, collection, getOperation)
...
//如果在查询条件下，第四个参数必须不能为空
function getInfoFromCollection(xmlDoc, xmlNode, collection, getOperation)
{
	var obj = new Object();
			...
			(currObj.dataSrc == xmlNode.parentNode.nodeName))
		{
			...
			if (strValue.length > 0) //判断输入框是否空
			{
				...
				if (xmlNode.nodeName == "WHERE") //判断条件的where 部分有效
				{
					if (obj.oper != null)
					{
						var TempNode = appendNode(xmlDoc, xmlNode, obj.dataFld, obj.value)
					
						appendAttr(xmlDoc, TempNode, "operator", obj.oper);
					}
				}
				else
				{	//在insert,update时有效
					...
					{
						appendNode(xmlDoc, xmlNode, strFld, obj.value);
					}
					else
			...
			if ((currObj.oldValue) && (currObj.oldValue != strValue))
			{
				...
				appendNode(xmlDoc, xmlNode, currObj.dataFld, obj.value);
			}
...
function createSelectDoc(DisplayFields, RowCount, tableName, conditions, getOperation)
{
	...
	appendAttr(xmlSelDoc, schemaNode, "rowcount", RowCount);
	
	var tableNode = findTableFromXml(xmlSelDoc, tableName, true);
	
	addDisplayFields(xmlSelDoc, tableNode, DisplayFields);
		
	if (arguments[3] != null)
		addConditionFields(xmlSelDoc, tableNode, conditions, getOperation);
...
function addFlagForServer(xmlDoc, Param)
{
	if ((xmlDoc != null) && (Param != ""))
	{
		...
		appendAttr(xmlDoc, Root, "param", Param);
	}
}
...
function findTableFromXml(xmlDoc, tableName, schemaFlag)
{
	...
	if (schemaFlag == true)
	{
		...
		if (tableNode == null)
		{
			var schemaNode = xmlDoc.selectSingleNode(".//SCHEMA");
			tableNode = xmlDoc.createNode(1, tableName, "");
	...
	else
	{
		...
		if (tableNode == null)
		{
			var schemaNode = xmlDoc.documentElement;
			tableNode = xmlDoc.createNode(1, tableName, "");
...
//增加在查询中要显示的字段
function addDisplayFields(xmlSelDoc, tableNode, DisplayFields)
{
	//增加display节点
	var displayNode = xmlSelDoc.createNode(1, "DISPLAY", "");
	
	tableNode.appendChild(displayNode);
	
	if (paramIsArray(DisplayFields) == true )
	{
		for (var i = 0;i < DisplayFields.length; i++)
		{
		   var TempNode = xmlSelDoc.createNode(1, DisplayFields[i], "");
		   
		   displayNode.appendChild(TempNode);
		}
	}
	else
	{	
		...
		{
		   var TempNode = xmlSelDoc.createNode(1, DisplayFields, "");
		   
		   if (arguments.length == 4)
				TempNode.text = arguments[3];
...
//增加条件的字段(Where部分)
//第三个参数可以为数据集和字符串,如果是数据集后面是回调函数（可选），如果是字符串
//第三个参数是字段名，第四个是操作，第五个是数值，第六个是类型（可选）
//
function addConditionFields(xmlSelDoc, tableNode)
{	
	var conditionNode = tableNode.selectSingleNode("WHERE");
	
	if (conditionNode == null)
	{
		conditionNode = xmlSelDoc.createNode(1, "WHERE", "");
		tableNode.appendChild(conditionNode);
	}
	
	if (arguments.length == 4)
	{
		getInfoFromCollection(xmlSelDoc, conditionNode, arguments[2], arguments[3])
	}
	else
		if (arguments.length >= 5)
		{
			var TempNode = xmlSelDoc.createNode(1, arguments[2], "");
			TempNode.text = arguments[4];
			
			if ((arguments[3] != null) && (arguments[3] != ""))
			{		
				appendAttr(xmlSelDoc, TempNode, "operator", arguments[3]);
			}
			
			if (arguments.length == 6)
			{
				appendAttr(xmlSelDoc, TempNode, "type", arguments[5]);
			}
...
function createUpdateDoc(collection, tableName)
{
	...
	var node = appendNode(xmlDoc, tableNode, "SET");
	
	getInfoFromCollection(xmlDoc, node, collection);
	
	if (arguments.length > 3)
	{
		addConditionFields(xmlDoc, tableNode, arguments[2], arguments[3], arguments[4])
	}
	else
	{
		if (arguments.length == 3)
		{
			addConditionFields(xmlDoc, tableNode, collection, arguments[2]);
		}
		else
		{
			addConditionFields(xmlDoc, tableNode, collection);
		}
	}
...
function createUpdateDoc2(collection, tableName, getOperation, strWhereField, strOP, strWhereValue)
{
	...
	getInfoFromCollection(xmlDoc, node, collection, getOperation);

	if (strWhereField)
		addConditionFields(xmlDoc, tableNode, strWhereField, strOP, strWhereValue);
...
function createUpdateDocbyXSD(collection, tableName, xsdDict, getOperation)
{
	var xmlDoc = createDomDocument("<Update/>");	
	var tableNode = findTableFromXml(xmlDoc, tableName);
	var setNode = appendNode(xmlDoc, tableNode, "SET");
	var whereNode = appendNode(xmlDoc, tableNode, "WHERE");
		...
		{			
			...
			if (obj.value.length > 0) //判断输入框是否空
			{
				...
				if (getBoolParam(getAttribByDict(INFO_FILE, obj.dataFld, "isKey")))
					addConditionFields(xmlDoc, tableNode, obj.dataFld, "=", obj.value);
				else
					appendNode(xmlDoc, setNode, obj.dataFld, obj.value);
			}
			...
			if ((currObj.oldValue) && (currObj.oldValue != obj.value))
			{
				...
				appendNode(xmlDoc, setNode, currObj.dataFld, obj.value);
			}
		}
	}
	return xmlDoc;
...
//Update的SET部分
function getUpdateFormCollection(xmlDoc, xmlNode, collection)
{
	for (i = 0; i < collection.length; i++)
	{
		var currObj = collection(i);

		if ((isUndefined(typeof(currObj.dataFld)) == false) && 
			(currObj.dataFld.length > 0) && 
			(currObj.dataSrc == xmlNode.parentNode.nodeName) && 
			(currObj.value != currObj.oldValue))
		{
			var TempNode = xmlDoc.createNode(1, currObj.dataFld, "");
			TempNode.text = originalValue(currObj);
...
function getConditionXml(elemContainer, callback)
{
	...
	for (var i = 0; i < elemContainer.all.length; i++)
	{
		...
		if (curObj.tagName == "INPUT" || curObj.tagName == "SELECT" || curObj.tagName == "dnInput")
...
{
	var node = appendNode(rootCondition.ownerDocument, rootCondition, strFld);
...
function createDelXSDDoc(collection, tableName, callback)
{
	...
	for (var i = 0; i < collection.length; i++)
	{
		...
		if (getBoolParam(getAttribByDict(INFO_FILE, curObj.dataFld, "isKey")))
		{
			if (callback)
				calback(curObj);
			appendNode(xmlDoc, whereNode, curObj.dataFld, originalValue(curObj));
		}
	}
...
function addXmlNodeIntoDic(xmlDoc, sName, sType, sCaption)
{
	...
	appendAttr(xmlDoc, xmlTempNode, "name", sName);
	
	appendAttr(xmlDoc, xmlTempNode, "type", sType);
	
	appendAttr(xmlDoc, xmlTempNode, "caption", sCaption);
...
//向数据集中动态添加数据
function addXmlNodeIntoDSXml(xmlDoc, xmlNode, sName, sValue)
{	
	while (xmlNode != null)
	{
		if (xmlNode.selectSingleNode(sName) == null)
		{
			appendNode(xmlDoc, xmlNode, sName, sValue);
		}
