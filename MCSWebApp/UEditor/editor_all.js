...
///randy added for 下载图片
UE.commands['downloadimage'] = {
	execCommand: function () {
		UEditorWrapperBridge.getInstance().SetActiveEditorWrapper(this.Wrapper);
		var activeEditor = UEditorWrapperBridge.getInstance().getActiveEditor();

		activeEditor.downLoadAndUpload(null, true);
	...
	me.commands['wordimage'] = {
		execCommand: function () {
			...
			for (var i = 0, ci; ci = images[i++]; ) {
				var url = ci.getAttribute("word_img")// || ci.getAttribute("data_ue_src");
...
(function () {
	function getClipboardData(callback) {
		...
		//randy wang add 调用控件Bridge和控件通信
		var wrapperBridge = UEditorWrapperBridge.getInstance();
		wrapperBridge.SetActiveEditorWrapper(this.Wrapper);
		...
		switch (node.tag) {
			case 'img':
				...
				if (node.attributes.src && /^(?:file)/.test(node.attributes.src)) {
					...
					var activeEditorWrapper = UEditorWrapperBridge.getInstance().getActiveEditor();

					if (activeEditorWrapper)
						activeEditorWrapper._imageNodes.push(node);
...
(function () {
			...
			return function (editor, iframeUrl, title) {
				...
				var ui = new editorui.Button({
					...
					onclick: function () {
						if (cmd == "insertimage") {
							if (!editor.Wrapper)
								UEditorWrapperBridge.getInstance().SetActiveEditorWrapper(editor.container.parentNode.id);
							else
								UEditorWrapperBridge.getInstance().SetActiveEditorWrapper(editor.Wrapper);
						}
						dialog.open();
					}
				});
	...
	editorui.WordImage = function (editor) {
		var ui = new editorui.Button({
			...
			onclick: function () {
				...
				if (editor.word_img && editor.word_img.length > 0) {// && window.confirm("是否转存本地图片？")) {
					//randy wang add 调用控件Bridge和控件通信
					var wrapperBridge = UEditorWrapperBridge.getInstance();
					if (!editor.Wrapper)
						wrapperBridge.SetActiveEditorWrapper(editor.container.parentNode.id);
					else
						wrapperBridge.SetActiveEditorWrapper(editor.container.parentNode.id);

					var uploadImgResult = wrapperBridge.manualUpload(editor.word_img);
	...
	//下载网络路径图片
	editorui.DownloadImage = function (editor, title) {
		title = title || editor.options.labelMap['downloadImage'] || '';
		var ui = new editorui.Button({
			className: 'edui-for-' + "insertimage",
			title: title,
			onclick: function () {
				editor.execCommand("downloadImage");
			}
		});
		editor.addListener('selectionchange', function () {

			var state = editor.queryCommandState('downloadImage');
			if (state == -1) {
				ui.setDisabled(true);
			} else {
				ui.setDisabled(false);
