...
namespace CIIC.HSR.TSP.WF.PermissionManager.ObjectDetail
{
    public class OguOmpProvider : IOguProvider
    {
        ...
        /// <returns>资源列表</returns>
        public List<Resource> GetAllResourcesByRoleCode(string roleCode)
        {
            List<Resource> result = new List<Resource>();
            var iResource = Containers.Default.Resolve<IWorkflowEngineBizlet>();
        ...
        private Resource TranslateResource(AAResourceBO aaResourceBo)
        {
        ...
        private Group TranlateGroup(AADomainRoleBO aaDomainRoleBo)
        {
            return new Group()
            {
                CodeName = aaDomainRoleBo.Code,
                Description = aaDomainRoleBo.LabelDescription,
                DisplayName = aaDomainRoleBo.LabelNameCn,
